...
class RoomType12(RoomBase):
    ...
    def roomEnd(self, args):
        ...
        :return:
        """
        DEBUG_MSG("[RoomType12 id %s]---------> roomEnd type(%s) args %s" % (self.id, type(args), args))
        info = {}
        max_score = -1
    ...
    def chapterStart(self, args):
        DEBUG_MSG("[RoomType12 id %s]---------> chapterStart args %s" % (self.id, args))
        cn = args["cn"]
        self.gameStart = True
        if cn == 0:
    ...
    def chapterEnd(self, args):
        DEBUG_MSG("[RoomType12 id %s]---------> chapterEnd args %s" % (self.id, args))
        # msg = json.dumps(args)
        cn = args["cn"]
