...
class RoomType2(RoomBase):
    ...
    def newChapter(self, maxPlayerCount=100):
        ...
        # 龙注数和
        _chapter["sumBetDragon"] = 0
        # 虎注数和
        _chapter["sumBetTiger"] = 0
        # 和注数和
    ...
    def newPlayer(self, accountEntity):
        ...
        # 龙注数和
        _player["sumBetDragon"] = 0
        # 虎注数和
        _player["sumBetTiger"] = 0
        # 和注数和
        _player["sumBetDraw"] = 0
        # 玩家剩余金币,用于计算玩家每轮下注金额是否大于剩余金币,因为玩家的gold属性是每局游戏结束的时候更新的,取的数据是不准确的
        _player["_bet_residue"] = _player["gold"]
    ...
    def updateMapping(self):
        ...
        _mapping_1 = updateMapping1(_win_history)
    ...
    def addPlayerInSD(self, _e_id):
        ...
        if _e.info["isBot"] == 1:
            _s_1 = []
            for i in range(0, random.randint(7, 12)):
                _s_1.append(1)
            for i in range(0, random.randint(7, 12)):
                _s_1.append(2)
            _s_1.append(3)
    ...
    def refreshRankingList(self):
        ...
        for i in _l:
            _e_id = i["entityId"]
            if _e_id not in KBEngine.entities.keys():
                continue
            _r_l.append(_e_id)
    ...
    def settlement(self):
        ...
        # 计算赌金池
        _sum_bet = 0
        _sum_bet += _chapter["sumBetDragon"]
        _sum_bet += _chapter["sumBetTiger"]
        _sum_bet += _chapter["sumBetDraw"]
        # 揭示底牌 判断输赢
        _card_1 = self.randomCardFromLib()
        _card_2 = self.randomCardFromLib()
        _r = RoomType2.compareCard(_card_1, _card_2)
        if _r > 0:
            _result = 1
        elif _r < 0:
            _result = 2
        ...
        for i in _players.keys():
            ...
            # 计算每个当前玩家要扣除的赌金
            _player_sum_bet = 0
            _player_sum_bet += _p["sumBetDragon"]
            _player_sum_bet += _p["sumBetTiger"]
            _player_sum_bet += _p["sumBetDraw"]
            _bets[i] = -_player_sum_bet
            # 加上赢取的金币
            if _result == 1:
                if _chapter["sumBetDragon"] == 0:
                    _v = 0
                else:
                    _v = (_p["sumBetDragon"] / _chapter["sumBetDragon"]) * _sum_bet
            elif _result == 2:
                if _chapter["sumBetDragon"] == 0:
                    _v = 0
                elif _chapter["sumBetTiger"] == 0:
                    _v = 0
                else:
                    _v = (_p["sumBetTiger"] / _chapter["sumBetTiger"]) * _sum_bet
            else:
                if _chapter["sumBetDragon"] == 0:
                    _v = 0
                elif _chapter["sumBetDraw"] == 0:
                    _v = 0
                else:
                    _v = (_p["sumBetDraw"] / _chapter["sumBetDraw"]) * _sum_bet
            _bets[i] += _v
        _playersHistory = self.statisticalData["playersHistory"]
        for k in _bets.keys():
            _gold_add = _bets[k]
        ...
        self.updateMapping()
        # 通知客户端底牌输赢信息
        self.callOtherClientsFunction("settlementInfo", [_card_1, _card_2, _result])
        # 通知客户端表格信息
    ...
    def betOperation(self, locationIndex, betType, bet):
        ...
        _bet_residue = _player["_bet_residue"]
        # 非本局玩家不能下注
        ...
        # 下注合法性检测
        if betType == 1:
            _player["sumBetDragon"] += bet
            _chapter["sumBetDragon"] += bet
        elif betType == 2:
            _player["sumBetTiger"] += bet
            _chapter["sumBetTiger"] += bet
        elif betType == 3:
        ...
        # 玩家剩余金额
        _bet_residue = _player["gold"] - _player["sumBetDragon"] - _player["sumBetTiger"] - _player["sumBetDraw"]
        _player["_bet_residue"] = _bet_residue
        # 通知客户端
    ...
    def reconnect(self, accountId):
        ...
        :return:
        """
        DEBUG_MSG('[RoomType2 id %i]------>reconnect %s' % (self.id, accountId))
    ...
    def retChapterInfo(self, accountId):
        ...
        self.callClientFunction(accountId, "Reconnect", [str(_chapter["chapterState"]),
                                                         ...
                                                         str(_gold), str(_chapter["sumBetDragon"]),
                                                         str(_chapter["sumBetTiger"]),
                                                         str(_chapter["sumBetDraw"])])
    ...
    def retLocations(self):
        ...
        for _p in _players.values():
            _e = _p["entity"]
            if _e.info["isBot"] == 0:
                DEBUG_MSG("roomtype2,players:%s,e%s" % (_players, _e))
                _id = _e.id
                _user_id = _e.info["userId"]
                _location_ = _p["locationIndex"]
                _e_s = _entityIds.copy()
                ...
                _location_info["locationIndexs"] = _l_s
                _json_data = json.dumps(_location_info)
                DEBUG_MSG("roomtype2 ,_e:%s,clientEntity:%s" % (_e, _e.clientEntity(self.id)))
                _e.clientEntity(self.id).retLocationIndexs(_json_data)
        ...
        if timerId == _timers["main"]:
            ...
            for b in _bots:
                ...
                if _r_0 < _r_p:
                    ...
                    # 额外下和的概率
                    self.betOperation(b["locationIndex"], 3, 1)
                elif _r_0 < _r_p + 3:
                    # 额外下和的概率
