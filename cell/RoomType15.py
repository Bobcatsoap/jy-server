...
class RoomType15(RoomBase):
    ...
    def addPlayerInSD(self, player):
        ...
        DEBUG_MSG('[RoomType15 id %i]------->addPlayerInSD player:%s.' % (self.info['roomId'], player))
    ...
    def changeChapterState(self, state):
        ...
        if _old == CHAPTER_STATE_INIT and state == CHAPTER_STATE_READY:
            ...
            DEBUG_MSG('[RoomType15 id %i]------->changeChapterState to %s' % (self.info['roomId'], state))
          ...
          """
            ...
            if self.xia_pao_type == 1:
                # 每局跑
                self.start_timer("xiaPaoTimer")
            elif self.xia_pao_type == 2:
                # 4局跑
                if self.cn % 4 == 0:
                    self.start_timer("xiaPaoTimer")
                else:
            ...
            elif self.xia_pao_type == 3:
                # 固定跑
                _players = _chapter[PLAYER_IN_GAME]
                for player in _players.values():
                    player[XIA_PAO_COUNT] = self.xia_pao_num
                    _args = {"entityId": player['entity'].id, "pao": self.xia_pao_num}
                    self.callOtherClientsFunction("PaoBack", _args)
                self.check_xia_pao_result(True)
    ...
    def settlement(self):
        ...
        if not _chapter[IS_HUANG_ZHUANG]:
            for hu_player in hu_players:
                DEBUG_MSG("[RoomType15] settlement HU_TYPE=%s" % _chapter[HU_TYPE])
                # 胡牌分数
                hu_score = 0
                # 跑分分数
                pao_score = 0
                ...
                # 计算胡牌分数和扣分范围
                if _chapter[IS_ZI_MO_HE]:
                    hu_score = self.zi_mo_score
                    hu_lose_range = no_hu_players.copy()

                    pao_score = hu_score
                    pao_lose_range = no_hu_players.copy()
                elif _chapter[IS_QIANG_GANG_HE]:
                    hu_score = self.dian_pao_score
                    # 开启点炮包大，扣一个人所有输家的钱
                    if self.dian_pao_bao_da:
                        for i in range(len(no_hu_players)):
                            hu_lose_range.append(self.get_player_by_location(_chapter[FANG_PAO_INDEX]))
                    else:
                        hu_lose_range = [self.get_player_by_location(_chapter[FANG_PAO_INDEX])]

                    pao_score = hu_score
                    pao_lose_range = [self.get_player_by_location(_chapter[FANG_PAO_INDEX])]
                ...
                if _chapter[IS_QIANG_GANG_HE]:
                    multiple += self.qiang_gang_multiple
                if multiple != 0:
                    hu_score *= multiple

                DEBUG_MSG('hu_player:%s,hu_score:%s' % (hu_player['entity'].info['name'], hu_score))

                banker_need_bao_da = True
                # 扣胡牌分
                for _p in hu_lose_range:
                    _hu_score = hu_score
                    if self.info['bankerAdd']:
                        # 如果庄家输牌，并且开启了点炮包大
                        if _p['locationIndex'] == _chapter[BANKER_LOCATION]:
                            _hu_score *= 2
                            if self.dian_pao_bao_da and not banker_need_bao_da:
                                _hu_score = int(_hu_score / 2)
                            # 庄家输钱时，点炮包大不翻倍
                            banker_need_bao_da = False
                        elif hu_player['locationIndex'] == _chapter[BANKER_LOCATION]:
                            _hu_score *= 2
                    DEBUG_MSG('name:%s,_hu_score:%s' % (_p['entity'].info["name"], _hu_score))
                ...
                if self.xia_pao_type != 0:
                    # 扣下跑分
                    for _p in pao_lose_range:
                        xia_pao_count = hu_player[XIA_PAO_COUNT] + _p[XIA_PAO_COUNT]
                        if xia_pao_count < 0:
                            xia_pao_count = 0
                        xia_pao_gold = xia_pao_count * pao_score
                        DEBUG_MSG('name:%s,xia pao:%s' % (_p['entity'].info["name"], xia_pao_gold))
                        _p['goldChange'] -= xia_pao_gold
                        hu_player['goldChange'] += xia_pao_gold

        # 如果没有荒庄，或者荒庄不荒杠开启，计算杠分
        if not _chapter[IS_HUANG_ZHUANG] or self.not_huang_gang:
            gang_pao_score = self.gang_di

            # 扣杠分
            gang_di = self.gang_di
            for _gang_p in _chapter[PLAYER_IN_GAME].values():
                ...
                for i in _gang_p[DA_MING_GANG_S].values():
                    ...
                    if self.info['bankerAdd']:
                        if _p['locationIndex'] == _chapter[BANKER_LOCATION] or \
                                _gang_p['locationIndex'] == _chapter[BANKER_LOCATION]:
                    ...
                    # 杠飘扣下跑分
                    if self.gang_pao:
                        xia_pao_count = _gang_p[XIA_PAO_COUNT] + _p[XIA_PAO_COUNT]
                        if xia_pao_count < 0:
                            xia_pao_count = 0
                        xia_pao_gold = xia_pao_count * gang_pao_score
                        DEBUG_MSG('name:%s,gang xia pao:%s' % (_p['entity'].info["name"], xia_pao_gold))
                        _p['goldChange'] -= xia_pao_gold
                        _gang_p['goldChange'] += xia_pao_gold
                        _p['mingGangGold'] -= xia_pao_gold
                        _gang_p['mingGangGold'] += xia_pao_gold
                ...
                for _ in _gang_p[AN_GANG_S]:
                    ...
                    for _p in _chapter[PLAYER_IN_GAME].values():
                        ...
                        if self.gang_pao:
                            ...
                            _p['goldChange'] -= xia_pao_gold
                            _gang_p['goldChange'] += xia_pao_gold
                            _p['anGangGold'] -= xia_pao_gold
                            _gang_p['anGangGold'] += xia_pao_gold

        # 赋值总金币改变
        for _p in players:
            _p['totalGoldChange'] += _p['goldChange']
            _p["entity"].update_score_control(_p['goldChange'])
            DEBUG_MSG('RoomType15 settlement totalGoldChange%s' % _p['totalGoldChange'])
        ...
        for p in players:
            ...
            end_msg.append(pm)
        DEBUG_MSG('[RoomType15 id %i]------------->chapterEnd end_msg=%s' % (self.info['roomId'], end_msg))
        for p in players:
    ...
    def total_settlement(self, is_disband=False):
        ...
        players = self.get_all_players_by_seat()
        DEBUG_MSG('[RoomType15 id %i]------------->totalSettlement msg=%s' % (self.info['roomId'], room_end_msg))
    ...
    def reconnection(self, accountEntityId):
        ...
        self.syncChapterNum(self.cn + 1)
        DEBUG_MSG('[RoomType15 id %s]------------->reconnection msg:%s' % (self.info['roomId'], users))
    ...
    def playerOperation(self, accountEntityId, jsonData):
        ...
        :return:
        """
        DEBUG_MSG('RoomType15 playerOperation accountEntityId:%s jsonData:%s' % (accountEntityId, jsonData))
    ...
    def onTimer(self, timerId, userArg):
        ...
        if timerId == _timers["main"]:
            ...
            _chapter['resOutPaiTimer'] = -1
            # 响应出牌，其他玩家超时后默认操作
            DEBUG_MSG('[RoomType15 id %i]------------->Timer(%s) 响应出牌,(%s)超时操作[guoOpt]' % (
            ...
            _chapter['qiangGangTimer'] = -1
            # 响应出牌，其他玩家超时后默认操作
            DEBUG_MSG('[RoomType15 id %i]------------->Timer(%s) 响应抢杠胡,(%s)超时操作[guoOpt]' % (
                self.info['roomId'], timerId, self.get_current_player()['locationIndex']))
            self.not_opt_players_operate_pass()
    ...
    def start_next_chapter(self):
        ...
        for v in _players.values():
            ...
            if self.xia_pao_type == 2 and self.cn % 4 != 0:
    ...
    def init_shou_pai(self):
        ...
        # 有人输的太多的话，需要发好牌
        is_in_rand_range = self.is_need_rand_score_control("RoomType15")
        luck_player = None
        if is_in_rand_range:
    ...
    def change_cur_player(self, player):
        ...
        if _chapter[CUR_PLAYER]:
            DEBUG_MSG('[RoomType15 id %i]------------->ChangeCurPlayer %s to %s' % (
    ...
    def check_current_player(self, player):
        ...
        if not self.is_player_in_players(player, allow_out_players):
            ...
            DEBUG_MSG('[RoomType15 id %i]------>Error: correct player: %s, current player: %s' % (
    ...
    def check_can_opt_after_deal(self, player, is_gang=False, is_gang_after_peng=False):
        ...
        DEBUG_MSG('[RoomType15 id %i]------------->FaPai ishu=%s' % (self.info['roomId'], _isHu))
        # 设置当前玩家可以胡牌
        player[CAN_HU] = _isHu
    ...
    def del_res_play_card_timer(self):
        _chapter = self.get_current_chapter()
        self.delTimer(_chapter['resOutPaiTimer'])
        DEBUG_MSG('[RoomType15 id %i]------------->删除响应出牌超时定时器(%s)' % (self.info['roomId'], _chapter['resOutPaiTimer']))
        _chapter['resOutPaiTimer'] = -1
    ...
    def xiao_ming_gang_opt(self, player, pai):
        DEBUG_MSG('[RoomType15 id %i]------------->xiaoMingGangOpt CurPlayer=%s, Pai=%s' % (
    ...
    def an_gang_opt(self, player, pai):
        DEBUG_MSG(
            '[RoomType15 id %i]------------->anGangOpt CurPlayer=%s, Pai=%s' % (
                self.info['roomId'], player['locationIndex'], pai))
        # 移除杠牌
    ...
    def zi_mo_hu_opt(self, player):
        DEBUG_MSG(
            '[RoomType15 id %i]------------->ziMoHuOpt CurPlayer=%s' % (self.info['roomId'], player['locationIndex']))
        _chapter = self.get_current_chapter()
    ...
    def chiOpt(self, player, pai):
        DEBUG_MSG(
            '[RoomType15 id %i]------------->chiOpt CurPlayer=%s, Pais=%s' % (
                self.info['roomId'], player['locationIndex'], pai))
        _chapter = self.get_current_chapter()
    ...
    def peng_opt(self, player, pai):
        DEBUG_MSG(
            '[RoomType15 id %i]------------->pengOpt CurPlayer=%s, Pai=%s' % (
                self.info['roomId'], player['locationIndex'], pai))
    ...
    def res_play_card_da_ming_gang_opt(self, player, pai):
        ...
        :return:
        """
        DEBUG_MSG('[RoomType15 id %i]------------->resOutPaiDaMingGangOpt CurPlayer=%s, Pai=%s' % (
            self.info['roomId'], player['locationIndex'], pai))
        _chapter = self.get_current_chapter()
        _check_results = _chapter[CHECK_RESULTS]
        for cr in _check_results:
    ...
    def da_ming_gang_opt(self, player, pai):
        DEBUG_MSG('[RoomType15 id %i]------------->daMingGangOpt CurPlayer=%s, Pai=%s' % (
            self.info['roomId'], player['locationIndex'], pai))
        self.check_opt_pai(pai)
        _chapter = self.get_current_chapter()
        other_index = self.get_wait_player()['locationIndex']
    ...
    def res_out_pai_dian_pao_hu_opt(self, player):
        ...
        """
        DEBUG_MSG(
            '[RoomType15 id %i]------------->resOutPaiDianPaoHuOpt CurPlayer=%s' % (
    ...
    def dian_pao_hu_opt(self, players):
        DEBUG_MSG('[RoomType15 id %i]------------->dianPaoHuOpt CurPlayer=%s' % (self.info['roomId'], players))
        # 要确保多个点炮胡操作顺序执行
        _chapter = self.get_current_chapter()
        # 如果没开一炮多响，只保留一个胡牌玩家
    ...
    def res_out_pai_qiang_gang_hu_opt(self, player):
        ...
        """
        DEBUG_MSG(
            '[RoomType15 id %i]------------->resOutPaiQiangGangHuOpt CurPlayer=%s' % (self.info['roomId'], player))
        _chapter = self.get_current_chapter()
        _check_results = _chapter[CHECK_RESULTS]
        for cr in _check_results:
            if player['entity'].id == cr[CHECK_PLAYER]['entity'].id:
    ...
    def qiang_gang_hu_opt(self, players):
        DEBUG_MSG('[RoomType15 id %i]------------->qiangGangHuOpt CurPlayer=%s' % (self.info['roomId'], players))
        _chapter = self.get_current_chapter()
        # 如果没开一炮多响，只保留一个胡牌玩家
        if not self.more_ring:
            can_hu = self.not_more_ring_filter(players)
    ...
    def hu(self, hu_players, fp_index=-1):
        ...
        DEBUG_MSG('[RoomType15 id %i]------------->HuOpt huIndex=%s, fangPaoIndex=%s' % (
            self.info['roomId'], hu_players, fp_index))
        self.end_round()
    ...
    @property
    def qiang_gang_multiple(self):
        return self.info['robGangHuMultiple']

    @property
    def zi_mo_score(self):
        return self.info['ziMoScore']

    @property
    def dian_pao_score(self):
        return self.info['dianPaoScore']
    ...
    def xia_pao_num(self):
        return self.info["redoubleNum"]

    @property
    def dian_pao_bao_da(self):
        return self.info['compensateMax']
    ...
    def on_account_cell_destroy(self, account_db_id):
        ...
        :return:
        """
        DEBUG_MSG('RoomType15 cell on_account_cell_destroy account_db_id:%s' % account_db_id)
        _chapter = self.get_current_chapter()
        all_death = True
        for p in _chapter[PLAYER_IN_GAME].values():
            if p['entity']:
    ...
    def get_special_multiple(self, special):
        ...
        if special == 1:
            return self.info['sevenMultiple']
        elif special == 2:
            return self.info['luxurySevenMultiple']
        elif special == 3:
            return self.info['sameColorMultiple']
        elif special == 4:
            return self.info['gangWinMultiple']
        elif special == 5:
            return 2
        return 0
