...
import mj.RoomType21Calculator
import PaiTypeUtil
import time
import Account
...
FA_PAI_AFTER_OPTS_CODE = OPT_OUT_V | OPT_XIAOMINGGANG_V | OPT_ANGANG_V | OPT_HU_ZIMO_V | OPT_TING_V | OPT_GUO_DEAL_V

# OUT_PAI_AFTER_OPTS = [OPT_GUO, OPT_CHI, OPT_PENG, OPT_DAMINGGANG, OPT_HU_DIANPAO]
# OPT_HU_QIANGGANG_V = 1 << 9
OUT_PAI_AFTER_OPTS_CODE = OPT_GUO_V | OPT_CHI_V | OPT_PENG_V | OPT_DAMINGGANG_V | OPT_HU_DIAN_PAO_V
# --------------------------------------------------------------------------------------------
#                            麻将检查结果
# --------------------------------------------------------------------------------------------
# 属于那个玩家
...
CHAPTER_STATE_DUN_LA = 3
CHAPTER_STATE_FAN_HUN = 4
CHAPTER_STATE_DEAL = 5
CHAPTER_STATE_PLAY = 6
CHAPTER_STATE_SETTLEMENT = 7
CHAPTER_STATE_TOTAL_SETTLEMENT = 8
...
class RoomConfig:
    ...
    @property
    def is_must_lacking_a_suit(self):
        """断门才能听,"""
        return self._info["duanMen"]

    @property
    def is_start_dun_la(self):
        """蹲拉开关"""
        return self._info["dunLa"]

    @property
    def is_zi_mo_after_pass_hu(self):
        """过胡只能自摸"""
        return True

    @property
    def get_cai_hua_type(self):
        """
        1:有财无花
        2：有花无财
        3：无财无花
        """
        return self._info["flowerType"]

    @property
    def is_zi_mo_after_an_gang(self):
        """
        暗杠全锁
        暗杠后所有玩家只能自摸
        """
...
class GamePlayer:
    ...
    def say_ting(self):
        ...
        报听
        """
        self._player[TING_TYPE] = 1

    def set_ting_card(self, card):
        """
        报听所出的牌
        """
        self._player[TING_TYPE] = 2
        self._player[ONE_TING_PAI] = card

    def is_waiting_ting_card(self):
        return self._player[TING_TYPE] == 1

    def is_tinged_card(self):
        return self._player[TING_TYPE] == 2

    @property
    def ting_state(self):
        return self._player[TING_TYPE]

    def add_flower_cards(self, flowers):
        self._player[HUA_PAI].extend(flowers)


class RoomType21(RoomBase):
    """
    """
    ...
    def newPlayer(self, accountEntity, total_gold_change=0, base_sync_gold_change=0, agree_disband=-1):
        ...
        _player[DUN_LA_NUM] = -1
        _player[ONE_TING_PAI] = None
        _player[CAN_TING_PAIS] = None
        # _player[CAN_NOT_PLAY_AFTER_TING] = None
        _player[TING_TYPE] = 0
    ...
    def addPlayerInSD(self, player):
        ...
        DEBUG_MSG('[RoomType21 id %i]------->addPlayerInSD player:%s.' % (self.id, player))
    ...
    def changeChapterState(self, state):
        ...
        if _old == CHAPTER_STATE_INIT and state == CHAPTER_STATE_READY:
            ...
            DEBUG_MSG('[RoomType21 id %i]------->changeChapterState to %s' % (self.id, state))
          ...
          """
            ...
            if self.room_cfg.xia_pao_type == 0:
                # 不带跑
                self.changeChapterState(CHAPTER_STATE_DUN_LA)
                return
        ...
        elif _old == CHAPTER_STATE_XIA_PAO and state == CHAPTER_STATE_DUN_LA:
            """
             蹲拉分设置
            """
            _chapter["chapterState"] = state
            ...
            if not self.room_cfg.is_start_dun_la:
                # 不带跑
                self.changeChapterState(CHAPTER_STATE_FAN_HUN)
                return

            self.callOtherClientsFunction("chapterStateChanged", [state])
            self.callOtherClientsFunction("StartDunLa", [])
            self.start_timer("dunLaTimer")

        elif _old == CHAPTER_STATE_DUN_LA and state == CHAPTER_STATE_FAN_HUN:
            _chapter["chapterState"] = state
            DEBUG_MSG('[RoomType21 id %i]------->changeChapterState to %s' % (self.id, state))
            self.callOtherClientsFunction("chapterStateChanged", [state])

            self.stop_timer("dunLaTimer")
            ...
            if self.info["maxChapterCount"] == self.cn + 1:
                ...
                self.stop_timer('autoPlayCardAfterTingTimer')
    ...
    def palyer_dun_la(self, account_entity_id, dun_la_num):  # timeout=False
        """
        用户选择蹲拉数，需要检查蹲拉数是否在允许范围内
        :param account_entity_id:
        :param dun_la_num:
        :return:
        """
        DEBUG_MSG('palyer_dun_la count:%s %s' % (account_entity_id, dun_la_num))
        if dun_la_num < 0 or dun_la_num > 3:
            return

        player = self.get_player_by_entity_id(account_entity_id)
        player[DUN_LA_NUM] = dun_la_num

        _args = {"entityId": account_entity_id, "dunLa": dun_la_num}
        self.callOtherClientsFunction("DunLaBack", _args)

        self.process_dun_la_result()

    def process_dun_la_result(self, timeout=False):
        """
        玩家蹲拉分选择
        超时，设置默认不拉
        :param timeout:
        :return:
        """
        if self.current_chapter.chapter_state != CHAPTER_STATE_DUN_LA:
            return
        # _chapter = self.current_chapter
        _players = self.current_chapter.players
        ...
        # 超时用户没选择，自主跑默认设为不跑；固定跑不用选择
        for player in _players.values():
            DEBUG_MSG('palyer_dun_la count:%s %s' % (player['entity'].id, player[DUN_LA_NUM]))
            if player[DUN_LA_NUM] == -1:
                if timeout:
                    player[DUN_LA_NUM] = 0
                    _args = {"entityId": player['entity'].id, "dunLa": 0}
                    self.callOtherClientsFunction("DunLaBack", _args)
                else:
                    return False
    ...
    def is_hu_one_card(self, player, _chapter):
        """
        是否掐张胡
        """
        shou_pais = player[SHOU_PAI].copy()
        shou_pais.remove(_chapter[CUR_PAI])
        hu_more = self.can_it_hu_more(shou_pais, _chapter[CUR_PAI])
        return not hu_more

    def is_have_three_same_card(self, player, _chapter):
        """
        暗卡：手里有3张一样的牌
        """
        shou_pais = player[SHOU_PAI].copy()
        shou_pais.remove(_chapter[CUR_PAI])
        for v in shou_pais:
            if shou_pais.count(v) >= 3:
                return True
        return False

    def is_no_peng_and_gang(self, hu_player):
        """
        门清：没有碰吃杠
        """
        if hu_player[PENG_S]:
        ...
        # if hu_player[CHIS]:
        #     return False
        if hu_player[XIAO_MING_GANG_S]:
            return False
        if hu_player[DA_MING_GANG_S]:
            return False
        return True

    def acc_score(self, score_detail, one_detail, hu_player, lose_player):
        hu_id = self.get_entity_id(hu_player)
        loser_id = self.get_entity_id(lose_player)
        if hu_id not in score_detail:
            score_detail[hu_id] = {}
        if loser_id not in score_detail:
            score_detail[loser_id] = {}

        for k, v in one_detail.items():
            if k not in score_detail[hu_id]:
                score_detail[hu_id][k] = v
            else:
                score_detail[hu_id][k] += v

            if k not in score_detail[loser_id]:
                score_detail[loser_id][k] = -v
            else:
                score_detail[loser_id][k] -= v
    ...
    def settlement(self):
        ...
        if not _chapter[IS_HUANG_ZHUANG]:
            ...
            for hu_player in hu_players:
                one_score_detail = {}
                DEBUG_MSG("[RoomType21] settlement HU_TYPE=%s" % _chapter[HU_TYPE])
                hu_score = 0
                # 存储扣鱼子分的玩家
                fish_range = []
                # 胡牌分数
                ...
                # 嘴分：报听，绝门，掐张，门清，暗卡 {"报听": 0, "断门": 0, "掐张": 0, "暗门": 0, "门清": 0, "自摸": 0}
                zui_score = 0
                if GamePlayer(hu_player).is_tinged_card():
                    zui_score += 1
                    zui_info.append("报听")
                suit_count = self.get_suit_count(hu_player, hu_player[SHOU_PAI])
                if 1 <= suit_count <= 2:
                    duan_count = 3 - suit_count
                    zui_score += duan_count
                    info = "断门"
                    if duan_count == 2:
                        info = "断门*2"
                    zui_info.append(info)
                if self.is_hu_one_card(hu_player, _chapter):
                    zui_score += 1
                    zui_info.append("掐张")
                if self.is_have_three_same_card(hu_player, _chapter):
                    zui_score += 1
                    zui_info.append("暗卡")
                if self.is_no_peng_and_gang(hu_player):
                    zui_score += 1
                    zui_info.append("门清")
                # 花分
                flower_score = len(hu_player[HUA_PAI])
                # 蹲拉分
                dun_la_score = 0
                if self.room_cfg.is_start_dun_la:
                    dun_la_score = hu_player[DUN_LA_NUM]
                # 自摸
                if _chapter[IS_ZI_MO_HE]:
                    # 自摸，胡分*2
                    hu_score *= 2
                ...
                # 七星
                seven_score = 0
                if mj.RoomType21Calculator.is_seven_pair(hu_player[SHOU_PAI]):
                    seven_score = 1
                ...
                for _lose_player in lose_range:
                    ...
                    if self.room_cfg.is_banker_add_di and (is_banker1 or is_banker2):
                        banker_jia_di = 1
                    append_count = xia_pao_count1 if xia_pao_count1 > xia_pao_count2 else xia_pao_count2
                    if append_count < 0:
                        append_count = 0
                    # append_count += banker_jia_di
                    # hu_pao_score = (append_count + 1) * hu_score
                    two_dun_la_score = 0
                    if self.room_cfg.is_start_dun_la and (is_banker1 or is_banker2):
                        if _chapter[IS_ZI_MO_HE]:
                            two_dun_la_score = _lose_player[DUN_LA_NUM] + dun_la_score
                            if two_dun_la_score < 0:
                                two_dun_la_score = 0
                    hu_pao_score = hu_score + zui_score + flower_score + two_dun_la_score + append_count + banker_jia_di + seven_score
                    ...
                    one_score_detail["huBase"] = hu_score
                    one_score_detail["zuiScore"] = zui_score
                    one_score_detail["flowerScore"] = flower_score
                    one_score_detail["dunLaScore"] = two_dun_la_score
                    one_score_detail["paoScore"] = append_count
                    one_score_detail["jiaDiScore"] = banker_jia_di
                    one_score_detail["sevenScore"] = seven_score
                    DEBUG_MSG(
                        'hu_score %s zui_score %s flower_score %s two_dun_la_score %s append_count %s banker_jia_di %s seven_score %s new_multiple %s' % (
                            hu_score, zui_score, flower_score, two_dun_la_score, append_count, banker_jia_di,
                            seven_score, new_multiple))

                    # 开启断门,未断门，输分翻倍
                    if self.room_cfg.is_must_lacking_a_suit:
                        is_statisfy_duan_men = self.is_statisfy_lacking_a_suit(_lose_player, _lose_player[SHOU_PAI])
                        if not is_statisfy_duan_men:
                            hu_pao_score *= 2

                    self.acc_score(score_detail, one_score_detail, hu_player, _lose_player)
                    DEBUG_MSG("hu_pao_score %s" % hu_pao_score)
            ...
            for _gang_p in _chapter[PLAYER_IN_GAME].values():
                ...
                for i in _gang_p[DA_MING_GANG_S].values():
                    ...
                    da_ming_gang_score = 1 * gang_di

                    # 开启断门,未断门，输分翻倍
                    if self.room_cfg.is_must_lacking_a_suit:
                        is_statisfy_duan_men = self.is_statisfy_lacking_a_suit(_p, _lose_player[SHOU_PAI])
                        if not is_statisfy_duan_men:
                            da_ming_gang_score *= 2

                    DEBUG_MSG("da_ming_gang_score %s" % da_ming_gang_score)
                ...
                for i in _gang_p[XIAO_MING_GANG_S].values():
                    ...
                    DEBUG_MSG("xiao_ming_score %s" % xiao_ming_score)
        ...
        for _p in players:
            ...
            DEBUG_MSG('RoomType21 settlement totalGoldChange%s' % _p['totalGoldChange'])
        ...
        for p in players:
            ...
            player_id = self.get_entity_id(p)
            if player_id in score_detail:
                for k, v in score_detail[player_id].items():
                    pm[k] = v
            else:
                pm["huBase"] = 0
                pm["zuiScore"] = 0
                ...
                pm["sevenScore"] = 0
            end_msg.append(pm)
        DEBUG_MSG('[RoomType21 id %i]------------->chapterEnd end_msg=%s' % (self.id, end_msg))
        for p in players:
    ...
    def total_settlement(self, is_disband=False):
        ...
        DEBUG_MSG('[RoomType21 id %i]------------->totalSettlement msg=%s' % (self.id, room_end_msg))
    ...
    def new_cards_lib(self):
        ...
        cai_hua_type = self.room_cfg.get_cai_hua_type
    ...
    def reconnection(self, accountEntityId):
        ...
        for p in players.values():
            ...
            can_not_play_cards = []
            ting_state = GamePlayer(p).ting_state
            if ting_state == 2:
                can_not_play_cards = p[SHOU_PAI].copy()
                if p[DAN_FANG] != -1:
                    remove_cards(can_not_play_cards, [p[DAN_FANG]])
            elif ting_state <= 1 and p[CAN_TING_PAIS]:
                can_not_play_cards = p[SHOU_PAI].copy()
                remove_cards(can_not_play_cards, p[CAN_TING_PAIS])
            ...
            users.append({
                ...
                'canNotPlayAfterTing': can_not_play_cards,
                'tingState': ting_state
            })

        self.syncChapterNum(self.cn + 1)
        # DEBUG_MSG('[RoomType21 id %s]------------->reconnection msg:%s' % (self.id, users))
        ...
        if _chapter[CUR_PLAYER] is not None:
            ...
            if player[DAN_FANG] != -1:
                ...
                _isHu = self.can_it_hu_match_duan_men(_shouPai, player=player)
                if _isHu:
                    if not GamePlayer(player).is_tinged_card():
                        # 设置当前玩家可以胡牌
                        _isHu = False
                player[CAN_HU] = _isHu
                ...
                is_ting = False
                if len(player[CAN_TING_PAIS]) and GamePlayer(player).ting_state <= 0:
                    player_allow_opt = player[ALLOW_OPT]
                    if OPT_TING_V & player_allow_opt > 0:
                        self.callClientFunction(player["entity"].id, CLIENT_FUNC_TING, "")
                        is_ting = True
                if len(_xmg) != 0 or len(_ag) != 0 or _isHu or is_ting:
    ...
    def get_one_valid_card_and_flowers(self, player):
        """
        取一张牌，有可能取到多张花牌
        """
        pai = None
    ...
    def get_one_card_and_flowers(self):
        """
        取一张牌，有可能取到多张花牌
        """
        pai = None
    ...
    def playerOperation(self, accountEntityId, jsonData):
        ...
        :return:
        """
        DEBUG_MSG('RoomType21 playerOperation accountEntityId:%s jsonData:%s' % (accountEntityId, jsonData))
        ...
        if not _check_player:
            DEBUG_MSG('is not _check_player1')
            return
        # 检查该玩家的操作
        _check_opt = self.checkOperation(_player, _operation_name)
        if not _check_opt:
            DEBUG_MSG('is not _check_opt2')
            return

        # 如果是出牌请求并且玩家不能再手动出牌，返回
        if _operation_name == OPT_OUT:
            out_pai = _args[0]
            if not self.play_card_check(_player, out_pai):
                DEBUG_MSG('is not ting card')
                return
            if GamePlayer(_player).is_tinged_card():
                DEBUG_MSG('is tinged can not play')
                return

        # 如果是听牌，不清空允许操作
        # if _operation_name != OPT_TING:
        ...
        if OPT_GUO_V == opt_code:
            if player_allow_opt & OPT_GUO_DEAL_V > 0:
                opt_code = OPT_GUO_DEAL_V
        ...
        if opt_code & FA_PAI_AFTER_OPTS_CODE == opt_code:
            ...
            if _operation_name == OPT_GUO:
                ...
                opt_v = OPT_DICT[OPT_HU_QIANG_GANG]
                if (player_allow_opt & opt_v) == opt_v:
                    _player['qiDianPao'] = True
    ...
    def operation_after_deal_card(self, _player, _operation_name, _args):
        ...
        # 小明杠
            ...
            self.del_play_card_timer()
            # self.del_auto_play_card_after_ting_timer()
            self.ting_opt(_player)
        elif OPT_HU_ZIMO == _operation_name:
            # 玩家操作，删除系统超时操作
            self.del_play_card_timer()
            # self.del_auto_play_card_after_ting_timer()
            self.zi_mo_hu_opt(_player)
        elif OPT_GUO == _operation_name:
            if GamePlayer(_player).is_tinged_card():
                out_pai = _player[SHOU_PAI][-1]
                self.play_card(_player, out_pai)
            else:
                _player[ALLOW_OPT] = OPT_OUT_V
    ...
    def clientReq(self, accountEntityId, jsonData):
        ...
        elif _func_name == 'SendDunLa':
            self.palyer_dun_la(accountEntityId, _args["dunLa"])
    ...
    def ret_player_in_room_info(self, accountId=-1):
        ...
        DEBUG_MSG('[RoomType21 roomid:%s] seat_to_player%s' % (self.info['roomId'], _player_seat))
        for k, v in _player_seat.items():
            _player_location = v['locationIndex']
            # 获取总金币
            total_gold_change = v['totalGoldChange']
    ...
    def start_timer(self, timer_name):
        ...
        if timer_name == "mainTimer":
            ...
            timeout_second = AUTO_TIME
            pass
        elif timer_name == "autoPlayCardAfterTingTimer":
            timeout_second = AUTO_PLAY_CARD_AFTER_TING
            pass
    ...
    def onTimer(self, timerId, userArg):
        ...
        if timerId == _chapter["mainTimer"]:
            ...
            _chapter['resOutPaiTimer'] = -1
            # 响应出牌，其他玩家超时后默认操作
            DEBUG_MSG('[RoomType21 id %i]------------->Timer(%s) 响应出牌,(%s)超时操作[guoOpt]' % (
            ...
            _chapter['qiangGangTimer'] = -1
            # 响应出牌，其他玩家超时后默认操作
            DEBUG_MSG('[RoomType21 id %i]------------->Timer(%s) 响应抢杠胡,(%s)超时操作[guoOpt]' % (
                self.id, timerId, self.get_current_player()['locationIndex']))
            self.not_opt_players_operate_pass()

        elif timerId == _chapter["xiaPaoTimer"]:
            self.process_xia_pao_result(True)
        elif timerId == _chapter["dunLaTimer"]:
            self.process_dun_la_result(True)
    ...
    def is_flower(self, card):
        if self.room_cfg.get_cai_hua_type == 1:
            return 31 <= card <= 33
        elif self.room_cfg.get_cai_hua_type == 2:
            return 34 <= card <= 41
        return False

    def replace_flower_with_normal(self, all_cards, all_flowers):
        """
        移除花牌，补普通牌
        """
        for cards in all_cards:
            ...
            for card in flowers:
                cards.remove(card)
                card, tmp_flowers = self.get_one_card_and_flowers()
                cards.append(card)
                new_flowers.extend(tmp_flowers)
            all_flowers.append(new_flowers)

    def init_shou_pai(self):
        _chapter = self.get_current_chapter()

        # 有人输的太多的话，需要发好牌
        is_in_rand_range = self.is_need_rand_score_control("RoomType21")
        luck_player = None
        if is_in_rand_range:
        ...
        all_flowers = []
        self.replace_flower_with_normal(all_cards, all_flowers)
        ...
        if luck_player:
            ...
            hun_zi_list = []
            good_card_index = mj.RoomType21Calculator.get_good_pai(all_cards, hun_zi_list)
            if good_card_index >= 0:
                luck_player[SHOU_PAI] = all_cards[good_card_index]
                if all_flowers[good_card_index]:
                    GamePlayer(luck_player).add_flower_cards(all_flowers[good_card_index])
                del all_cards[good_card_index]
                del all_flowers[good_card_index]
        ...
        for p in _chapter[ROUND]:
            if have_pai_player_id == p['entity'].id:
                self.deal_cards_to_player(p['locationIndex'], p[SHOU_PAI], p[HUA_PAI])
                continue
            p[SHOU_PAI] = all_cards[0]
            if all_flowers[0]:
                GamePlayer(p).add_flower_cards(all_flowers[0])
            del all_cards[0]
            del all_flowers[0]
            self.deal_cards_to_player(p['locationIndex'], p[SHOU_PAI], p[HUA_PAI])
        # DEBUG_MSG(_chapter[ROUND])

    def deal_cards_to_player(self, locationIndex, cards, hua_cards):
        """
        给玩家发牌
        :param locationIndex:
        :param cards:
        ...
        # self.otherClients.dealCardsToPlayer(locationIndex, cards, hua_cards)
        self.callOtherClientsFunction("dealCardsToPlayer",
                                      {"locationIndex": locationIndex, "cards": cards, "flowers": hua_cards})

        _chapter = self.get_current_chapter()
        _replay = {'locationIndex': locationIndex, 'otherIndex': -1, "opt": 'initPai', "card": cards.copy(),
                   "flowers": hua_cards.copy()}
    ...
    def change_cur_player(self, player):
        ...
        if _chapter[CUR_PLAYER]:
            DEBUG_MSG('[RoomType21 id %i]------------->ChangeCurPlayer %s to %s' % (
    ...
    def check_current_player(self, player):
        ...
        if not self.is_player_in_players(player, allow_out_players):
            ...
            DEBUG_MSG('[RoomType21 id %i]------>Error: correct player: %s, current player: %s' % (
    ...
    def checkOperation(self, player, operation_name):
        ...
        if OPT_GUO_V == opt_v:
            if allow_opt & OPT_GUO_DEAL_V > 0:
                opt_v = OPT_GUO_DEAL_V
    ...
    def deal_one_card(self, player, is_gang, dian_gang_pai=None):
        ...
        :return:
        """
        pai, flowers = self.get_one_valid_card_and_flowers(player)
        if pai is None:
            return
        _chapter = self.get_current_chapter()
        ...
        if flowers:
            GamePlayer(player).add_flower_cards(flowers)
        # 打印手牌
        # 设置单放
        player[DAN_FANG] = pai
        # 操作次数+1
        ...
        # 同步剩余牌的数量
        # 同步发牌
        self.syncOpt(player['locationIndex'], SYNCOPT_FAPAI, pai, -1, flowers)
    ...
    def check_can_opt_after_deal(self, player, is_gang=False, is_gang_after_peng=False, dian_gang_pai=None):
        ...
        _ag = self.check_an_gang(player)
        # _ting_pais = player[CAN_TING_PAIS]
        _isHu = self.can_it_hu_match_duan_men(_hand_cards, player=player)
        DEBUG_MSG('[RoomType21 id %i]------------->FaPai ishu=%s' % (self.id, _isHu))
        if _isHu:
            if not GamePlayer(player).is_tinged_card():
                # 设置当前玩家可以胡牌
                _isHu = False
        ...
        # player[CAN_TING] = True if _ting_pais else False
        ...
        if len(_ag) != 0:
            ...
            all_not = False
        # if _ting_pais:
        #     self.callClientFunction(player["entity"].id, CLIENT_FUNC_TINGPAI, _ting_pais)
        #     all_not = False
        ...
        if len(player[CAN_TING_PAIS]) and GamePlayer(player).ting_state <= 0:
            ...
            all_not = False
        if not all_not:
            player[ALLOW_OPT] |= OPT_GUO_DEAL_V
            self.callClientFunction(player["entity"].id, CLIENT_FUNC_GUO, "")
        ...
        if player['entity'].client_death and self.info['timeDown'] != 0:
            ...
            out_pai = cur_player[SHOU_PAI][-1]
            self.operation_after_deal_card(cur_player, OPT_OUT, [out_pai])
        # elif GamePlayer(player).is_tinged_card() and all_not:
        ...
        # 如果玩家在线，开启计时器
            ...
            # 客户端报听，且没碰、杠、胡
            if GamePlayer(player).is_tinged_card() and all_not:
                _chapter['playCardTimer'] = self.addTimer(AUTO_PLAY_CARD_AFTER_TING, 0, 0)
                _chapter['deadLine'] = time.time() + AUTO_PLAY_CARD_AFTER_TING
                self.sync_timer_count_down(AUTO_PLAY_CARD_AFTER_TING)
                return
    ...
    def operation_over(self, timeout=False, operator=None, operation_name=None):
        ...
        DEBUG_MSG('[RoomType21] operation_over is_qiang_gang_hu %s' % is_qiang_gang_hu)
        for cr in check_results:
    ...
    def syncOpt(self, locationIndex, opt, pai, otherIndex, flowers=None):
        _chapter = self.get_current_chapter()
        sync_info = {"locationIndex": locationIndex, "opt": opt, "pai": pai, "otherIndex": otherIndex}
        if opt == SYNCOPT_FAPAI:
            sync_info["flowers"] = flowers if flowers else []
        elif SYNCOPT_OUTPAI == opt:
            sync_info["isTingPlay"] = False
            if pai == 99:
                pai = _chapter[CUR_PAI]
                sync_info["isTingPlay"] = True
                sync_info["pai"] = pai
        self.callOtherClientsFunction("syncOpt", sync_info)

        # 增加回放
        out_pai_replay = {'locationIndex': locationIndex, 'otherIndex': otherIndex, "opt": opt, "card": [pai]}
        if opt == SYNCOPT_FAPAI:
            out_pai_replay["flowers"] = flowers.copy() if flowers else []
    ...
    def del_res_play_card_timer(self):
        _chapter = self.get_current_chapter()
        DEBUG_MSG('[RoomType21 id %i]------------->删除响应出牌超时定时器(%s)' % (self.id, _chapter['resOutPaiTimer']))
        self.stop_timer('resOutPaiTimer')
    ...
    # --------------------------------------------------------------------------------------------
    # ---------------------  发牌之后操作  ---------------------
    def play_card_check(self, player, out_pai):
        # 移除手牌,加入出牌区
        _hand_cards = player[SHOU_PAI]
        if out_pai not in _hand_cards:
            return False

        # 听牌后只能出一张牌，出完牌不允许出其他手牌
        if GamePlayer(player).is_waiting_ting_card():
            if out_pai not in player[CAN_TING_PAIS]:
                self.callClientFunction(player['entity'].id, 'refreshChapter', {})
                return False
        elif GamePlayer(player).is_tinged_card():
            # 听牌后，不能换手中的牌，
            if player[DAN_FANG] > 0 and player[DAN_FANG] != out_pai:
                self.callClientFunction(player['entity'].id, 'refreshChapter', {})
                return False
        return True
    ...
    def play_card(self, player, out_pai):
        ...
        # 设置报听所出的牌
        is_report_ting = False
        mask_out_pai = out_pai
        if GamePlayer(player).is_waiting_ting_card():
            GamePlayer(player).set_ting_card(out_pai)
            mask_out_pai = 99
            self.get_current_player()[OUT_PAIS].append(mask_out_pai)
            # 清空打出能听的牌
        ...
        # 同步出牌消息
        self.syncOpt(player['locationIndex'], SYNCOPT_OUTPAI, mask_out_pai, -1)
        # 设置不胡牌
        ...
        for i, p in enumerate(other_players):
            ...
            p[CAN_PENG] = False
            p[CAN_HU] = False
            _is_peng = check_peng(p[SHOU_PAI], out_pai)

            # 商丘麻将不允许吃
            _chi = list()

            can_da_ming_gang = self.check_da_ming_gang(p, out_pai)
            _isHu = self.can_it_hu_match_duan_men(p[SHOU_PAI], out_pai, player=p)
            if _isHu:
                if not self.can_dian_pao(p, out_pai):
                    _isHu = False
            ...
            if is_report_ting:
            ...
            elif GamePlayer(p).is_tinged_card():
                # 听牌后，杠牌必须还能听才允许杠
                if can_da_ming_gang:
                    tmp_hand_cards = p[SHOU_PAI].copy()
                    remove_cards(tmp_hand_cards, [out_pai])
                    if not self.is_ting_cards(tmp_hand_cards, p):
                        can_da_ming_gang = False
                ...
                # 没报听不能胡
                if not GamePlayer(p).is_tinged_card():
                    _isHu = False
    ...
    def xiao_ming_gang_opt(self, player, pai):
        DEBUG_MSG('[RoomType21 id %i]------------->xiaoMingGangOpt CurPlayer=%s, Pai=%s' % (
    ...
    def an_gang_opt(self, player, pai):
        DEBUG_MSG(
            '[RoomType21 id %i]------------->anGangOpt CurPlayer=%s, Pai=%s' % (self.id, player['locationIndex'], pai))
        # 移除杠牌
        ...
        # 增加牌局暗杠数
        _chapter = self.get_current_chapter()
        _chapter[AN_GANG_COUNT] += 1
        # 给此玩家发牌
        self.deal_one_card(player, True)

    def ting_opt(self, player):
        """
        """
        self.debug_msg('ting CurPlayer=%s' % (player['locationIndex']))
        _chapter = self.get_current_chapter()
        _players = _chapter[PLAYER_IN_GAME]
        _hand_cards = player[SHOU_PAI]
        ...
        # 找到听后能打的牌
        can_not_play_cards = _hand_cards.copy()
        remove_cards(can_not_play_cards, player[CAN_TING_PAIS])
        self.callClientFunction(_entity_id, 'canNotPlayCardsAfterTing', {'cards': can_not_play_cards})

    def zi_mo_hu_opt(self, player):
        DEBUG_MSG('[RoomType21 id %i]------------->ziMoHuOpt CurPlayer=%s' % (self.id, player['locationIndex']))
        _chapter = self.get_current_chapter()
    ...
    def check_qiang_gang_hu(self, player, pai):
        ...
        for op_player in other_players:
            can_hu = self.can_it_hu_match_duan_men(op_player[SHOU_PAI], pai, player=op_player)
            if can_hu:
                if not GamePlayer(op_player).is_tinged_card():
                    # 设置当前玩家可以胡牌
                    can_hu = False
                if not self.can_dian_pao(op_player, pai):
                    can_hu = False
            op_player[CAN_HU] = can_hu
    ...
    def chiOpt(self, player, pai):
        DEBUG_MSG(
            '[RoomType21 id %i]------------->chiOpt CurPlayer=%s, Pais=%s' % (self.id, player['locationIndex'], pai))
        _chapter = self.get_current_chapter()
    ...
    def peng_opt(self, player, pai):
        DEBUG_MSG(
            '[RoomType21 id %i]------------->pengOpt CurPlayer=%s, Pai=%s' % (self.id, player['locationIndex'], pai))
        ...
        # 如果有小明杠，暗杠，通知操作
            ...
            self.check_can_opt_after_deal(player)
    ...
    def res_play_card_da_ming_gang_opt(self, player, pai):
        ...
        :return:
        """
        DEBUG_MSG('[RoomType21 id %i]------------->resOutPaiDaMingGangOpt CurPlayer=%s, Pai=%s' % (
            self.id, player['locationIndex'], pai))
        _chapter = self.get_current_chapter()
        _check_results = _chapter[CHECK_RESULTS]
        for cr in _check_results:
    ...
    def da_ming_gang_opt(self, player, pai):
        DEBUG_MSG('[RoomType21 id %i]------------->daMingGangOpt CurPlayer=%s, Pai=%s' % (
            self.id, player['locationIndex'], pai))
        self.check_opt_pai(pai)
        _chapter = self.get_current_chapter()
        other_index = self.get_wait_player()['locationIndex']
    ...
    def res_out_pai_dian_pao_hu_opt(self, player):
        ...
        """
        DEBUG_MSG(
            '[RoomType21 id %i]------------->resOutPaiDianPaoHuOpt CurPlayer=%s' % (self.id, player['locationIndex']))
        _chapter = self.get_current_chapter()
        _check_results = _chapter[CHECK_RESULTS]
        for cr in _check_results:
            if player['entity'].id == cr[CHECK_PLAYER]['entity'].id:
    ...
    def dian_pao_hu_opt(self, players):
        DEBUG_MSG('[RoomType21 id %i]------------->dianPaoHuOpt CurPlayer=%s' % (self.id, players))
        # 要确保多个点炮胡操作顺序执行
        _chapter = self.get_current_chapter()
    ...
    def res_out_pai_qiang_gang_hu_opt(self, player):
        ...
        :return:
        """
        DEBUG_MSG('[RoomType21 id %i]------------->resOutPaiQiangGangHuOpt CurPlayer=%s' % (self.id, player))
        _chapter = self.get_current_chapter()
        _check_results = _chapter[CHECK_RESULTS]
        for cr in _check_results:
            if player['entity'].id == cr[CHECK_PLAYER]['entity'].id:
    ...
    def qiang_gang_hu_opt(self, players):
        DEBUG_MSG('[RoomType21 id %i]------------->qiangGangHuOpt CurPlayer=%s' % (self.id, players))
        _chapter = self.get_current_chapter()
        # 记录一下抢杠和
    ...
    def hu_pai(self, hu_players, fp_index=-1):
        ...
        DEBUG_MSG('[RoomType21 id %i]------------->HuOpt huIndex=%s, fangPaoIndex=%s' % (self.id, hu_players, fp_index))
        self.end_round()
    ...
    def hu_tip(self, account_entity_id, play_card):
        ...
        if play_card in hand_cards:
            ...
            for card in can_hu_range:
                ...
                temp.append(card)
                # 平胡
                can_hu = self.can_it_hu_match_duan_men(temp, player=player)
                if can_hu:
    ...
    def hu_anything(self, hand_cards):
        ...
        for v in range(34):
            ...
            can_hu = self.can_it_hu_match_duan_men(new_hand_cards)
    ...
    def on_account_cell_destroy(self, account_db_id):
        ...
        :return:
        """
        DEBUG_MSG('RoomType21 cell on_account_cell_destroy account_db_id:%s' % account_db_id)
        _chapter = self.get_current_chapter()
        all_death = True
        for p in _chapter[PLAYER_IN_GAME].values():
            if p['entity']:
    ...
    def get_cards_suit_count(self, cards, suits):
        """
        检查牌的门数
        """
        # suits = set()
    ...
    def get_suit_count(self, player, _hand_cards):
        suits = set()
        self.get_cards_suit_count(_hand_cards, suits)
        _cards = list(player[PENG_S].keys())
        self.get_cards_suit_count(_cards, suits)
        _cards = list(player[CHIS].keys())
        self.get_cards_suit_count(_cards, suits)
        _cards = list(player[XIAO_MING_GANG_S].keys())
        self.get_cards_suit_count(_cards, suits)
        _cards = list(player[DA_MING_GANG_S].keys())
        self.get_cards_suit_count(_cards, suits)
        _cards = player[AN_GANG_S]
        self.get_cards_suit_count(_cards, suits)
        return len(suits)

    def is_statisfy_lacking_a_suit(self, player, _hand_cards):
        """
        是否满足断门设置
        """
        if self.room_cfg.is_must_lacking_a_suit:
            return self.get_suit_count(player, _hand_cards) <= 2
        else:
            return True

    def can_it_hu_match_duan_men(self, shou_pais, pai=-1, hu_result=None, player=None):
        if player:
            if not self.is_statisfy_lacking_a_suit(player, shou_pais):
                # DEBUG_MSG("can_it_hu_match_duan_men not duan men %s, %s" % (shou_pais, pai))
                return False
        return self.can_it_hu(shou_pais, pai, hu_result)
    ...
    def can_it_hu(self, shou_pais, pai=-1, hu_result=None):
        ...
        # 七星判断
        ret = mj.RoomType21Calculator.is_seven_pair(new_show_pais)
        if ret:
        ...
        ret = mj.RoomType21Calculator.can_hu_contain_lai_zi(new_show_pais, [])
        # if self.room_cfg.hun_pai_type == 0:
        # 不带混
        # ret = mj.RoomType21Calculator.can_hu_pai(new_show_pais)
        # elif self.room_cfg.hun_pai_type == 1:
        #     # 单混
        #     hun_zi_list = self.current_chapter.hun_pais
        #     hun_pai_count = mj.RoomType21Calculator.hun_pai_count(shou_pais, hun_zi_list)
        #     if hun_pai_count == 4:
        #         ret = True
        #         temp_hu_result = HU_RESULT_FOUR_HUN
        #     else:
        #         ret = mj.RoomType21Calculator.can_hu_contain_lai_zi(shou_pais, hun_zi_list, pai)

        # elif self.room_cfg.hun_pai_type == 2:
        #     # 双混, 只能自摸
        #     if pai == -1:
        #         hun_zi_list = self.current_chapter.hun_pais
        #         ret = mj.RoomType21Calculator.can_hu_contain_lai_zi(new_show_pais, hun_zi_list, pai)

        if ret and hu_result is not None:
            hu_result.append(temp_hu_result)
        return ret

    def can_it_hu_more(self, shou_pais, pai):
        """
        判断是否能胡更多(掐张胡)
        shou_pais: 目前是差一张的情况
        """
        cards_hu_range = self.can_hu_cards_range(shou_pais)
        if pai in cards_hu_range:
            cards_hu_range.remove(pai)
        # 加入一张牌，看是否会胡
        can_hu_cards = []
        for card in cards_hu_range:
            ...
            if can_hu:
                can_hu_cards.append(card)
        return len(can_hu_cards) > 0
    ...
    def hu_type(self, player):
        ...
        hu_result = []
        self.can_it_hu_match_duan_men(cards, hu_result=hu_result, player=player)
    ...
    def can_dian_pao(self, player, out_card):
        ...
        :return:
        """
        if self.room_cfg.is_zi_mo_hu:
            return False

        if player['qiDianPao']:
            return False

        if self.room_cfg.is_zi_mo_after_an_gang:
            _chapter = self.get_current_chapter()
            if _chapter[AN_GANG_COUNT] > 0:
                return False

        if self.room_cfg.is_zi_mo_after_pass_hu:
            if player[QI_HU]:
                return False
    ...
    def can_hu_cards_range(self, hand_cards):
        ...
        :return:
        """
        hand_cards = mj.RoomType21Calculator.analyse1(hand_cards)
        card_range = []
        mj.RoomType21Calculator.select_perhaps_pai(hand_cards, card_range)
    ...
    def share_to_wx(self, account_id):
        if self.info['roomType'] == 'card':
            title = '商丘麻将房间号：' + str(self.info["roomId"])
        elif self.info['teaHouseId'] != -1:
            title = '商丘麻将房间' + ',冠名赛id:' + str(self.info['teaHouseId'])
        else:
            title = '商丘麻将房间'
        ...
        if self.room_cfg.get_cai_hua_type == 1:
            ...
            red_magic = "有花无财"
        elif self.room_cfg.get_cai_hua_type == 3:
            red_magic = "无财无花"
        red_magic = red_magic
        # single_color = "????"
        base_score = '底分：' + str(
    ...
    def is_ting_cards(self, hand_cards, player):
        """
        手牌是否是听牌
        """
        ...
        for card in cards_hu_range:
            ...
            # 记录胡牌类型
            hu_result_type = []
            can_hu = self.can_it_hu_match_duan_men(temp, hu_result=hu_result_type, player=player)
            if can_hu:
                return True
        return False
    ...
    def ting_tip(self, account_entity_id):
        ...
        if player['entity'].id != account_entity_id:
            return
        if GamePlayer(player).is_tinged_card():
            return
...
def remove_cards(cards, remove_cards):
    """
    从一副牌中移除所有属于另一副牌的牌
    """
