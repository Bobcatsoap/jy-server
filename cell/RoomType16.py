...
import mj.RoomType15Calculator
...
class RoomType16(RoomBase):
    ...
    def onEnter(self, accountEntityId):
        ...
        DEBUG_MSG('[Room id %i]------->onEnter account = %i.' % (self.info['roomId'], _accountEntity.id))
    ...
    def removePlayer(self, _player):
        ...
        if _player:
            ...
            DEBUG_MSG('[Room id%s] removePlayer locationIndex %s,players:%s' % (
                self.info['roomId'], _locationIndex, _players))
    ...
    def newPlayer(self, accountEntity, total_gold_change=0, base_sync_gold_change=0, agree_disband=-1):
        ...
        # 通知base房间玩家人数变化
        self.base.cellToBase({"func": "playersCount", "count": _chapter["playersCount"]})
        # DEBUG_MSG('[Room id%s] newPlayer locationIndex %s' % (self.info['roomId'], _locationIndex))
    ...
    def addPlayerInSD(self, player):
        ...
        DEBUG_MSG('[RoomType16 id %i]------->addPlayerInSD player:%s.' % (self.info['roomId'], player))
    ...
    def changeChapterState(self, state):
        ...
        if _old == CHAPTER_STATE_INIT and state == CHAPTER_STATE_READY:
            ...
            DEBUG_MSG('[RoomType16 id %i]------->changeChapterState to %s' % (self.info['roomId'], state))
            ...
            _chapter["chapterState"] = state
            DEBUG_MSG('[RoomType14 id %i]------->changeChapterState to %s' % (self.info['roomId'], state))
            if self.xia_pao_type == 0:
                # 不带跑
            ...
            self.callOtherClientsFunction("FanHun", {'mingPai': 31, 'hunPai': [31]})
            ...
            self.delTimer(_chapter['fanHunTimer'])
            _chapter['fanHunTimer'] = -1
    ...
    def init_ma_info(self, hu_players):
        """
        获取扎马信息
        :return:
        """
        ...
        _chapter = self.chapters[self.cn]
        # 剩余牌
        remain_cards = _chapter[CARDS_LIB].copy()
        random.shuffle(remain_cards)
        # 一码全中模式：
        # 如果红中赖子开启，剔除剩余牌中除了红中的字牌
        # 如果红中赖子关闭，剔除所有字牌
        if self.ma_count == 1:
            if self.magic_red_switch:
                remain_cards = [x for x in remain_cards if x <= 26 or x == 31]
            else:
                remain_cards = [x for x in remain_cards if x <= 26]

        DEBUG_MSG('get_ma_info remain_cards%s' % remain_cards)
        # 剩余牌不够一人扎马,所有人的马牌都为剩余牌
        if len(remain_cards) < self.ma_count:
            for p in hu_players:
                p['maCards'] = remain_cards
        # 剩余的足够多少个玩家扎马
        else:
            # 能扎马的玩家数
            can_ma_count = int(len(remain_cards) / self.ma_count)
            # 遍历能扎马的玩家，每人取得对应数的马牌
            for index, p in enumerate(hu_players):
                # 在能扎马的范围内时，直接拿牌
                if index <= can_ma_count - 1:
                    p['maCards'] = remain_cards[index * self.ma_count:(index + 1) * self.ma_count]
                # 不在能扎马的范围内时，先洗牌再拿牌
                else:
                    shuffle_cards = remain_cards.copy()
                    random.shuffle(shuffle_cards)
                    DEBUG_MSG('player:%s,shuffle_cards:%s' % (p, shuffle_cards))
                    p['maCards'] = shuffle_cards[:self.ma_count]
        ...
        for p in hu_players:
            ...
            if len(p['maCards']) == 0:
                continue
            # 一码全中时，按照字牌的大小算分，红中9分
            if self.ma_count == 1:
                # 一码全中，扎中的牌
                p['maCorrectCards'] = [p['maCards'][0]]
                ma_card = p['maCorrectCards']
                ma_number = self.get_card_number(ma_card[0])
                # 红中9分
                if ma_number == -1:
                    p['maScore'] = 9
                else:
                ...
                # 不是一码全中时
                p['maCorrectCards'] = [x for x in p['maCards'] if self.ma_correct(x)]
                # 每扎中一个，获得一分
                for _ in p['maCorrectCards']:
                    p['maScore'] += 1
            DEBUG_MSG('get_ma_info maCards:%s,maScore:%s,maCorrectCards:%s' % (p['maCards'],
                                                                               p['maScore'],
                                                                               p['maCorrectCards']))

    def ma_correct(self, card):
        """
        2,4,6 模式下是否扎中
        :param card:
        :return:
        """
        number = self.get_card_number(card)
        if number == 1 or number == 5 or number == 9 or (self.magic_red_switch and card == 31):
            return True
        return False
    ...
    def loser_or_winner_is_banker(self, loser, winner):
        """
        赢家或输家有一方是庄家（加倍）
        :param loser:
        :param winner:
        ...
        """
        # 未开启庄闲不扣庄闲分
        if not self.info['bankerAdd']:
            return False
        _chapter = self.chapters[self.cn]
        if loser['locationIndex'] == _chapter[BANKER_LOCATION] or winner['locationIndex'] == _chapter[BANKER_LOCATION]:
            return True
        return False
    ...
    def settlement(self):
        ...
        if not _chapter[IS_HUANG_ZHUANG]:
            for hu_player in hu_players:
                DEBUG_MSG("[RoomType16] settlement HU_TYPE=%s" % _chapter[HU_TYPE])
                # 胡牌分数
                hu_score = 0
                # 存储输胡牌分的玩家
                hu_lose_range = []
                # 码分
                ma_score = hu_player['maScore']
                # 扣扎马分范围
                ma_lose_range = []
                # 计算胡牌分数和扣分范围
                if _chapter[IS_ZI_MO_HE]:
                    hu_score = self.base_score * 2
                    hu_lose_range = no_hu_players.copy()
                    if self.dian_gang_bao_da and _chapter[IS_DIAN_GANG_HU]:
                        # 找到点杠的人,点杠的人包其他人的分
                        for i in range(len(no_hu_players)):
                            ma_lose_range.append(self.get_player_by_location(_chapter[DIAN_GANG_PLAYER]))
                    else:
                        ma_lose_range = hu_lose_range.copy()
                elif _chapter[IS_QIANG_GANG_HE]:
                    hu_score = self.base_score
                    hu_lose_range = [self.get_player_by_location(_chapter[FANG_PAO_INDEX])]
                    # 抢杠大包,输家为杠牌的人，输家同时包其他人(除了该赢家之外的人)的码分
                    if self.qiang_gang_bao_da:
                        for i in range(len(_chapter[PLAYER_IN_GAME]) - 1):
                            ma_lose_range.append(self.get_player_by_location(_chapter['qiangDaMingGangPlayer']))
                    # 正常抢杠胡，输家为点炮者
                    else:
                        ma_lose_range = hu_lose_range.copy()
                ...
                for _p in hu_lose_range:
                    _hu_score = hu_score
                    if self.loser_or_winner_is_banker(_p, hu_player):
                        _hu_score += self.base_score
                    if self.dian_gang_bao_da and _chapter[IS_DIAN_GANG_HU]:
                        # 点杠大包，扣点杠人的分
                ...
                # 扣扎马分，扎马分=扎中的马数*扎马分
                for _p in ma_lose_range:
                    _p['goldChange'] -= ma_score * self.info['zhaMaScore']
                    hu_player['goldChange'] += ma_score * self.info['zhaMaScore']

        # 如果没有荒庄，或者荒庄不荒杠开启，计算杠分
        if not _chapter[IS_HUANG_ZHUANG] or self.not_huang_gang:
            # 扣杠分
            gang_di = self.base_score
            for _gang_p in _chapter[PLAYER_IN_GAME].values():
                # 大明杠扣触发杠的人
                for i in _gang_p[DA_MING_GANG_S].values():
                    # 大明杠分
                    da_ming_gang_score = gang_di * self.da_ming_gang_multiple
                    _p = self.get_player_by_location(i)
                    DEBUG_MSG('name:%s,da ming gang:%s' % (_p['entity'].info["name"], da_ming_gang_score))
                ...
                for i in _gang_p[XIAO_MING_GANG_S].values():
                    ...
                    _p = self.get_player_by_location(i)
                    DEBUG_MSG('name:%s,xiao ming:%s' % (_p['entity'].info["name"], xiao_ming_score))
                ...
                for _ in _gang_p[AN_GANG_S]:
                    ...
                    for _p in _chapter[PLAYER_IN_GAME].values():
                        if self.is_same_player(_p, _gang_p):
                            continue
                        an_score = gang_di * 2
                        DEBUG_MSG('name:%s,an:%s' % (_p['entity'].info["name"], an_score))
        ...
        for _p in players:
            ...
            DEBUG_MSG('RoomType16 settlement totalGoldChange%s' % _p['totalGoldChange'])
        ...
        for p in players:
            pm = {"locationIndex": p['locationIndex'],
                  ...
                  'maCorrectCards': p['maCorrectCards'],
                  'maCards': p['maCards'],
            ...
            end_msg.append(pm)
        DEBUG_MSG('[RoomType16 id %i]------------->chapterEnd end_msg=%s' % (self.info['roomId'], end_msg))
        for p in players:
    ...
    @property
    def da_ming_gang_multiple(self):
        _chapter = self.chapters[self.cn]
    ...
    def total_settlement(self, is_disband=False):
        ...
        players = self.get_all_players_by_seat()
        DEBUG_MSG('[RoomType16 id %i]------------->totalSettlement msg=%s' % (self.info['roomId'], room_end_msg))
    ...
    def write_chapter_replay(self):
        ...
        for c in range(0, chapter_record_max_count):
            ...
            for k, v in _player_seat.items():
                ...
                zha_ma_info[int(v['locationIndex'])] = {'maCards': v['maCards'],
                                                        'maCorrectCards': v['maCorrectCards']}
            _replay_data[c]['playerInfo'] = player_info
            _replay_data[c]['zhaMaInfo'] = zha_ma_info
    ...
    def write_chapter_info_to_db(self):
        ...
        DEBUG_MSG('[Room id %i]------>writeChapterInfoToDB, chapterInfo %s ' % (self.info['roomId'], _chapter_info))
    ...
    def new_cards_lib(self):
        ...
        # DEBUG_MSG('random_cards_with_time cards_lib%s' % _l)
    ...
    def reconnection(self, accountEntityId):
        ...
        self.syncChapterNum(self.cn + 1)
        DEBUG_MSG('[RoomType16 id %s]------------->reconnection msg:%s' % (self.info['roomId'], users))
    ...
    def playerOperation(self, accountEntityId, jsonData):
        ...
        :return:
        """
        DEBUG_MSG('RoomType16 playerOperation accountEntityId:%s jsonData:%s' % (accountEntityId, jsonData))
    ...
    def onTimer(self, timerId, userArg):
        ...
        if timerId == _timers["main"]:
            # DEBUG_MSG('[Room id %s]------------->onTimer main %s' % (self.info['roomId'], timerId))
            _chapter_state = _chapter["chapterState"]
            _players = _chapter["players"]
            _playersCount = len(_chapter[PLAYER_IN_GAME])
            if _chapter_state == 0:
            ...
            _chapter['resOutPaiTimer'] = -1
            # 响应出牌，其他玩家超时后默认操作
            DEBUG_MSG('[RoomType16 id %i]------------->Timer(%s) 响应出牌,(%s)超时操作[guoOpt]' % (
                self.info['roomId'], timerId, self.get_current_player()['locationIndex']))
            self.not_opt_players_operate_pass()
        elif timerId == _chapter['qiangGangTimer']:
            self.delTimer(timerId)
            _chapter['qiangGangTimer'] = -1
            # 响应出牌，其他玩家超时后默认操作
            DEBUG_MSG('[RoomType16 id %i]------------->Timer(%s) 响应抢杠胡,(%s)超时操作[guoOpt]' % (
                self.info['roomId'], timerId, self.get_current_player()['locationIndex']))
            self.not_opt_players_operate_pass()
    ...
    def init_shou_pai(self):
        ...
        # # 发牌
        # for p in players:
        #     DEBUG_MSG('init_shou_pai shou_pai:%s' % p[SHOU_PAI])
        #     for i in range(0, 13):
        #         # 添加手牌
        #         _c = self.get_card()
        ...
        # 有人输的太多的话，需要发好牌
        is_in_rand_range = self.is_need_rand_score_control("RoomType16")
        luck_player = None
        if is_in_rand_range:
    ...
    def deal_cards_to_player(self, locationIndex, cards):
        ...
        DEBUG_MSG('deal_cards_to_player locationIndex:%s,cards:%s' % (locationIndex, cards))
    ...
    def change_cur_player(self, player):
        ...
        if _chapter[CUR_PLAYER]:
            DEBUG_MSG('[RoomType16 id %i]------------->ChangeCurPlayer %s to %s' % (
                self.info['roomId'], _chapter[CUR_PLAYER]['locationIndex'], player['locationIndex']))
    ...
    def check_current_player(self, player):
        ...
        if not self.is_player_in_players(player, allow_out_players):
            ...
            DEBUG_MSG('[RoomType16 id %i]------>Error: correct player: %s, current player: %s' % (
                self.info['roomId'], allow_location_index, player['locationIndex']))
            return False
        return True
    ...
    def check_can_opt_after_deal(self, player, is_gang=False, is_gang_after_peng=False, is_da_ming_gang=False):
        ...
        DEBUG_MSG('[RoomType16 id %i]------------->FaPai ishu=%s' % (self.info['roomId'], _isHu))
        # 设置当前玩家可以胡牌
        player[CAN_HU] = _isHu
        ...
        for cr in check_results:
            ...
            if _cpo == OPT_HU_DIAN_PAO_V or _cpo == OPT_HU_QIANGGANG_V:
                ...
                if _cpo == OPT_HU_DIAN_PAO_V and da_ming_gang_player:
                    ...
                    DEBUG_MSG('operation_over qiangDaMingGangPlayer:%s' % _chapter['qiangDaMingGangPlayer'])
                can_other_opt = False
                # 找到所有的胡牌玩家
    ...
    def del_res_play_card_timer(self):
        _chapter = self.get_current_chapter()
        self.delTimer(_chapter['resOutPaiTimer'])
        DEBUG_MSG('[RoomType16 id %i]------------->删除响应出牌超时定时器(%s)' % (self.info['roomId'], _chapter['resOutPaiTimer']))
        _chapter['resOutPaiTimer'] = -1
    ...
    def xiao_ming_gang_opt(self, player, pai):
        DEBUG_MSG('[RoomType16 id %i]------------->xiaoMingGangOpt CurPlayer=%s, Pai=%s' % (
            self.info['roomId'], player['locationIndex'], pai))
    ...
    def an_gang_opt(self, player, pai):
        DEBUG_MSG(
            '[RoomType16 id %i]------------->anGangOpt CurPlayer=%s, Pai=%s' % (
                self.info['roomId'], player['locationIndex'], pai))
        # 移除杠牌
    ...
    def zi_mo_hu_opt(self, player):
        DEBUG_MSG(
            '[RoomType16 id %i]------------->ziMoHuOpt CurPlayer=%s' % (self.info['roomId'], player['locationIndex']))
        _chapter = self.get_current_chapter()
    ...
    def chiOpt(self, player, pai):
        DEBUG_MSG(
            '[RoomType16 id %i]------------->chiOpt CurPlayer=%s, Pais=%s' % (
                self.info['roomId'], player['locationIndex'], pai))
        _chapter = self.get_current_chapter()
    ...
    def peng_opt(self, player, pai):
        DEBUG_MSG(
            '[RoomType16 id %i]------------->pengOpt CurPlayer=%s, Pai=%s' % (
                self.info['roomId'], player['locationIndex'], pai))
    ...
    def res_play_card_da_ming_gang_opt(self, player, pai):
        ...
        :return:
        """
        DEBUG_MSG('[RoomType16 id %i]------------->resOutPaiDaMingGangOpt CurPlayer=%s, Pai=%s' % (
            self.info['roomId'], player['locationIndex'], pai))
        _chapter = self.get_current_chapter()
        _check_results = _chapter[CHECK_RESULTS]
        for cr in _check_results:
    ...
    def da_ming_gang_opt(self, player, pai):
        DEBUG_MSG('[RoomType16 id %i]------------->daMingGangOpt CurPlayer=%s, Pai=%s' % (
            self.info['roomId'], player['locationIndex'], pai))
        self.check_opt_pai(pai)
        _chapter = self.get_current_chapter()
        other_index = self.get_wait_player()['locationIndex']
        ...
        #
        DEBUG_MSG('DIAN_GANG_PLAYER locationIndex:%s' % other_index)
        # 销毁check_results
        self.destroy_check_result(player)
        # 同步大明杠消息
    ...
    def res_out_pai_dian_pao_hu_opt(self, player):
        ...
        """
        DEBUG_MSG(
            '[RoomType16 id %i]------------->resOutPaiDianPaoHuOpt CurPlayer=%s' % (
                self.info['roomId'], player['locationIndex']))
        _chapter = self.get_current_chapter()
        _check_results = _chapter[CHECK_RESULTS]
        for cr in _check_results:
            if player['entity'].id == cr[CHECK_PLAYER]['entity'].id:
    ...
    def dian_pao_hu_opt(self, players):
        DEBUG_MSG('[RoomType16 id %i]------------->dianPaoHuOpt CurPlayer=%s' % (self.info['roomId'], players))
        # 要确保多个点炮胡操作顺序执行
        _chapter = self.get_current_chapter()
        # 如果没开一炮多响，只保留一个胡牌玩家
    ...
    def not_more_ring_filter(self, hu_players):
        ...
        if not self.more_ring:
            if len(hu_players) > 1:
                ...
                DEBUG_MSG('not_more_ring_filter %s,%s' % (min_distance, min_location_index))
    ...
    def res_out_pai_qiang_gang_hu_opt(self, player):
        ...
        """
        DEBUG_MSG(
            '[RoomType16 id %i]------------->resOutPaiQiangGangHuOpt CurPlayer=%s' % (self.info['roomId'], player))
        _chapter = self.get_current_chapter()
        _check_results = _chapter[CHECK_RESULTS]
        for cr in _check_results:
            if player['entity'].id == cr[CHECK_PLAYER]['entity'].id:
    ...
    def qiang_gang_hu_opt(self, players):
        DEBUG_MSG('[RoomType16 id %i]------------->qiangGangHuOpt CurPlayer=%s' % (self.info['roomId'], players))
        _chapter = self.get_current_chapter()
        # 如果没开一炮多响，只保留一个胡牌玩家
        if not self.more_ring:
            can_hu = self.not_more_ring_filter(players)
    ...
    def hu(self, hu_players, fp_index=-1):
        ...
        if not _chapter[IS_HUANG_ZHUANG]:
            # 如果没有荒庄，计算扎马分
            self.init_ma_info(hu_players)
            zha_ma_info = {}
            for p in _chapter[PLAYER_IN_GAME].values():
                if self.is_player_in_players(p, hu_players):
                    zha_ma_info[p['locationIndex']] = {'maCards': p['maCards'], 'maCorrectCards': p['maCorrectCards']}
            self.callOtherClientsFunction('ZhaMaPlayers', {'zhaMaInfo': zha_ma_info})
        self.end_round()
    ...
    def xia_pao_type(self):
        ...
        """
        if self.info["redoubleType"] == 0:
            return 0
        elif self.info["redoubleType"] == -1:
            return 1 if self.info["redoubleWay"] == 0 else 2
        elif self.info["redoubleType"] == 1:
            return 3
        else:
            pass
    ...
    def on_account_cell_destroy(self, account_db_id):
        ...
        :return:
        """
        DEBUG_MSG('RoomType16 cell on_account_cell_destroy account_db_id:%s' % account_db_id)
        _chapter = self.get_current_chapter()
        all_death = True
        for p in _chapter[PLAYER_IN_GAME].values():
            if p['entity']:
    ...
    def get_special_multiple(self, special):
        ...
        :return:
        """
        # 特殊规则：1：七星对  2：豪华七星对  3:清一色  4：杠上花 5:四混
        if special == 1:
            return 1
        elif special == 2:
            return 1
    ...
    def can_hu(self, cards, cards_type=None, pai=-1):
        ...
        if self.magic_red_switch:
            wild_hu, j_258 = mj.RoomType15Calculator.can_hu_contain_lai_zi(cards, [31], pai)
        else:
            normal_hu, j_258 = mj.RoomType15Calculator.can_hu_contain_lai_zi(cards, [], pai)
        # 平胡
        hu = normal_hu or wild_hu
        if hu:
            hu_type = 0
        # 七星对
        seven_hu = mj.RoomType15Calculator.is_seven_pair(new_cards)
    ...
    def hu_type(self, player):
        ...
        hu_type = []
        seven_hu = mj.RoomType15Calculator.is_seven_pair(cards)
        DEBUG_MSG('hu_type cards:%s,seven_hu%s' % (cards, seven_hu))
        # 七星对判断
        if seven_hu == 1 or seven_hu == 2:
        ...
        if _chapter[IS_GANG_SHANG_KAI_HUA]:
            hu_type.append(4)
        if cards.count(31) == 4 and self.magic_red_switch:
            hu_type.append(5)
        return hu_type
    ...
    def share_to_wx(self, account_id):
        ...
        con = str('%s,%s,%s' % (chapter_count, red_magic, base_score))
    ...
    def ting_tip(self, account_entity_id):
        ...
        DEBUG_MSG('ting_tip can_hu_range:%s' % can_hu_range)
        # 1.移除要出的牌      2.加入任意牌     3.移除杠牌      4.判断能不能胡    5.判断能不能次
    ...
    def can_hu_cards_range(self, hand_cards):
        ...
        :return:
        """
        hand_cards = mj.RoomType15Calculator.analyse(hand_cards)
        card_range = []
        mj.RoomType15Calculator.select_perhaps_pai(hand_cards, card_range, self.magic_red_switch)
