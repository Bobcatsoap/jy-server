...
import mj.RoomType22Calculator
import PaiTypeUtil
import time
import Account
...
class RoomType22(RoomBase):
    ...
    def addPlayerInSD(self, player):
        ...
        DEBUG_MSG('[RoomType22 id %i]------->addPlayerInSD player:%s.' % (self.id, player))
    ...
    def changeChapterState(self, state):
        ...
        if _old == CHAPTER_STATE_INIT and state == CHAPTER_STATE_READY:
            ...
            DEBUG_MSG('[RoomType22 id %i]------->changeChapterState to %s' % (self.id, state))
    ...
    def get_zi_mo_score(self, player, _chapter):
        """
        点炮、自摸游戏类型：自摸分
        123:246:515：掐张自摸分为3,6,15；不掐张为：2,4,10
         # 0: 点炮 # 1：自摸 # 2: 123 # 3: 246 # 4: 515
        """
        hu_type = self.room_cfg.hu_type
        if hu_type == 0 or hu_type == 1:
            return self.room_cfg.zi_mo_score
        else:
            hand_pais = player[SHOU_PAI].copy()
            hand_pais.remove(_chapter[CUR_PAI])
            hu_more = self.can_it_hu_more(hand_pais, _chapter[CUR_PAI])
            if hu_type == 2:
                return 2 if hu_more else 3
            elif hu_type == 3:
                return 4 if hu_more else 6
            elif hu_type == 4:
                return 10 if hu_more else 15
        return 0

    def get_dian_pao_score(self):
        """
        点炮、自摸游戏类型：点炮分
        123:246:515：分别扣2,4,10
         # 0: 点炮 # 1：自摸 # 2: 123 # 3: 246 # 4: 515
        """
        hu_type = self.room_cfg.hu_type
        if hu_type == 0 or hu_type == 1:
            return self.room_cfg.dian_pao_score
        else:
            if hu_type == 2:
                return 2
            elif hu_type == 3:
    ...
    def get_gang_base_score(self):
        """
        点炮、自摸游戏类型：杠底分
        123:246:515：分别扣1,2,5
         # 0: 点炮 # 1：自摸 # 2: 123 # 3: 246 # 4: 515
        """
        hu_type = self.room_cfg.hu_type
        if hu_type == 0 or hu_type == 1:
            return self.room_cfg.gang_di_score
        else:
            if hu_type == 2:
                return 1
            elif hu_type == 3:
    ...
    def settlement(self):
        ...
        if not _chapter[IS_HUANG_ZHUANG]:
            ...
            for hu_player in hu_players:
                DEBUG_MSG("[RoomType22] settlement HU_TYPE=%s" % _chapter[HU_TYPE])
                hu_base_score = 0
                # 存储扣分的玩家
                fish_range = []
                # 胡牌分数
                if _chapter[IS_ZI_MO_HE]:
                    hu_base_score = self.get_zi_mo_score(hu_player, _chapter)
                    fish_range = no_hu_players.copy()
                    da_hu = True
                elif _chapter[IS_QIANG_GANG_HE]:
                    hu_base_score = self.get_dian_pao_score()
                ...
                # 123,246,515，七星对胡牌时，点炮按自摸处理
                # if _chapter[IS_QIANG_GANG_HE] or _chapter[IS_DIAN_PAO_HE]:
                #     if self.room_cfg.hu_type >= 2:
                #         if self.room_cfg.is_seven_pair_can_hu and mj.RoomType22Calculator.is_seven_pair(hu_player[SHOU_PAI]):
                #             hu_base_score = self.get_zi_mo_score(hu_player, _chapter)
                #             fish_range = no_hu_players.copy()
                #             da_hu = True
                ...
                # 扣除胡分、跑分
                _score_multiple = self.room_cfg.score_multiple
                ...
                for _lose_player in lose_range:
                    ...
                    hu_pao_score = (append_count + 1) * hu_base_score * _score_multiple

                    # if da_hu:
                    #     hu_pao_score = (append_count+1) * hu_base_score * _score_multiple
                    # else:
                    #     hu_pao_score = (append_count + hu_base_score) * _score_multiple
                    ...
                    DEBUG_MSG('hu_pao_score:%s %s, xia_pao_count: %s %s ' % (
                        hu_pao_score, hu_base_score, xia_pao_count1, xia_pao_count2))
            ...
            # 扣多少杠分
            gang_di = self.get_gang_base_score()
        ...
        for _p in players:
            ...
            DEBUG_MSG('RoomType22 settlement totalGoldChange%s' % _p['totalGoldChange'])
        ...
        for p in players:
            ...
            end_msg.append(pm)
        DEBUG_MSG('[RoomType22 id %i]------------->chapterEnd end_msg=%s' % (self.id, end_msg))
        for p in players:
    ...
    def total_settlement(self, is_disband=False):
        ...
        DEBUG_MSG('[RoomType22 id %i]------------->totalSettlement msg=%s' % (self.id, room_end_msg))
    ...
    def reconnection(self, accountEntityId):
        ...
        self.syncChapterNum(self.cn + 1)
        # DEBUG_MSG('[RoomType22 id %s]------------->reconnection msg:%s' % (self.id, users))
    ...
    def playerOperation(self, accountEntityId, jsonData):
        ...
        :return:
        """
        DEBUG_MSG('RoomType22 playerOperation accountEntityId:%s jsonData:%s' % (accountEntityId, jsonData))
    ...
    def ret_player_in_room_info(self, accountId=-1):
        ...
        DEBUG_MSG('[RoomType22 roomid:%s] seat_to_player%s' % (self.info['roomId'], _player_seat))
        for k, v in _player_seat.items():
            _player_location = v['locationIndex']
            # 获取总金币
            total_gold_change = v['totalGoldChange']
    ...
    def onTimer(self, timerId, userArg):
        ...
        if timerId == _chapter["mainTimer"]:
            ...
            _chapter['resOutPaiTimer'] = -1
            # 响应出牌，其他玩家超时后默认操作
            DEBUG_MSG('[RoomType22 id %i]------------->Timer(%s) 响应出牌,(%s)超时操作[guoOpt]' % (
            ...
            _chapter['qiangGangTimer'] = -1
            # 响应出牌，其他玩家超时后默认操作
            DEBUG_MSG('[RoomType22 id %i]------------->Timer(%s) 响应抢杠胡,(%s)超时操作[guoOpt]' % (
                self.id, timerId, self.get_current_player()['locationIndex']))
            self.not_opt_players_operate_pass()
    ...
    def init_shou_pai(self):
        ...
        # 有人输的太多的话，需要发好牌
        is_in_rand_range = self.is_need_rand_score_control("RoomType22")
        luck_player = None
        if is_in_rand_range:
        ...
        if luck_player:
            ...
            good_card_index = mj.RoomType22Calculator.get_good_pai(all_cards, hun_zi_list)
        ...
        # 给其他人发牌
        # all_cards[0] = [11, 7, 13, 13, 25, 21, 11, 21, 9, 7, 24, 9, 25]
        for p in _chapter[ROUND]:
    ...
    def change_cur_player(self, player):
        ...
        if _chapter[CUR_PLAYER]:
            DEBUG_MSG('[RoomType22 id %i]------------->ChangeCurPlayer %s to %s' % (
    ...
    def check_current_player(self, player):
        ...
        if not self.is_player_in_players(player, allow_out_players):
            ...
            DEBUG_MSG('[RoomType22 id %i]------>Error: correct player: %s, current player: %s' % (
    ...
    def check_can_opt_after_deal(self, player, is_gang=False, is_gang_after_peng=False, dian_gang_pai=None):
        ...
        DEBUG_MSG('[RoomType22 id %i]------------->FaPai ishu=%s' % (self.id, _isHu))
        # 设置当前玩家可以胡牌
        player[CAN_HU] = _isHu
    ...
    def operation_over(self, timeout=False, operator=None, operation_name=None):
        ...
        DEBUG_MSG('[RoomType22] operation_over is_qiang_gang_hu %s' % is_qiang_gang_hu)
        for cr in check_results:
    ...
    def del_res_play_card_timer(self):
        _chapter = self.get_current_chapter()
        DEBUG_MSG('[RoomType22 id %i]------------->删除响应出牌超时定时器(%s)' % (self.id, _chapter['resOutPaiTimer']))
        self.stop_timer('resOutPaiTimer')
    ...
    def play_card(self, player, out_pai):
        ...
        for i, p in enumerate(other_players):
            ...
            if b_hu:
                if not self.is_allow_hu_more():
                    if self.can_it_hu_more(p[SHOU_PAI], out_pai):
                        b_hu = False
    ...
    def xiao_ming_gang_opt(self, player, pai):
        DEBUG_MSG('[RoomType22 id %i]------------->xiaoMingGangOpt CurPlayer=%s, Pai=%s' % (
    ...
    def an_gang_opt(self, player, pai):
        DEBUG_MSG(
            '[RoomType22 id %i]------------->anGangOpt CurPlayer=%s, Pai=%s' % (self.id, player['locationIndex'], pai))
        # 移除杠牌
    ...
    def zi_mo_hu_opt(self, player):
        DEBUG_MSG('[RoomType22 id %i]------------->ziMoHuOpt CurPlayer=%s' % (self.id, player['locationIndex']))
        _chapter = self.get_current_chapter()
    ...
    def check_qiang_gang_hu(self, player, pai):
        ...
        for op_player in other_players:
            ...
            if can_hu:
                if not self.is_allow_hu_more():
                    if self.can_it_hu_more(op_player[SHOU_PAI], pai):
                        can_hu = False
    ...
    def chiOpt(self, player, pai):
        DEBUG_MSG(
            '[RoomType22 id %i]------------->chiOpt CurPlayer=%s, Pais=%s' % (self.id, player['locationIndex'], pai))
        _chapter = self.get_current_chapter()
    ...
    def peng_opt(self, player, pai):
        DEBUG_MSG(
            '[RoomType22 id %i]------------->pengOpt CurPlayer=%s, Pai=%s' % (self.id, player['locationIndex'], pai))
    ...
    def res_play_card_da_ming_gang_opt(self, player, pai):
        ...
        :return:
        """
        DEBUG_MSG('[RoomType22 id %i]------------->resOutPaiDaMingGangOpt CurPlayer=%s, Pai=%s' % (
            self.id, player['locationIndex'], pai))
        _chapter = self.get_current_chapter()
        _check_results = _chapter[CHECK_RESULTS]
        for cr in _check_results:
    ...
    def da_ming_gang_opt(self, player, pai):
        DEBUG_MSG('[RoomType22 id %i]------------->daMingGangOpt CurPlayer=%s, Pai=%s' % (
            self.id, player['locationIndex'], pai))
        self.check_opt_pai(pai)
        _chapter = self.get_current_chapter()
        other_index = self.get_wait_player()['locationIndex']
    ...
    def res_out_pai_dian_pao_hu_opt(self, player):
        ...
        """
        DEBUG_MSG(
            '[RoomType22 id %i]------------->resOutPaiDianPaoHuOpt CurPlayer=%s' % (self.id, player['locationIndex']))
        _chapter = self.get_current_chapter()
        _check_results = _chapter[CHECK_RESULTS]
        for cr in _check_results:
            if player['entity'].id == cr[CHECK_PLAYER]['entity'].id:
    ...
    def dian_pao_hu_opt(self, players):
        DEBUG_MSG('[RoomType22 id %i]------------->dianPaoHuOpt CurPlayer=%s' % (self.id, players))
        # 要确保多个点炮胡操作顺序执行
        _chapter = self.get_current_chapter()
    ...
    def res_out_pai_qiang_gang_hu_opt(self, player):
        ...
        :return:
        """
        DEBUG_MSG('[RoomType22 id %i]------------->resOutPaiQiangGangHuOpt CurPlayer=%s' % (self.id, player))
        _chapter = self.get_current_chapter()
        _check_results = _chapter[CHECK_RESULTS]
        for cr in _check_results:
            if player['entity'].id == cr[CHECK_PLAYER]['entity'].id:
    ...
    def qiang_gang_hu_opt(self, players):
        DEBUG_MSG('[RoomType22 id %i]------------->qiangGangHuOpt CurPlayer=%s' % (self.id, players))
        _chapter = self.get_current_chapter()
        # 记录一下抢杠和
    ...
    def hu_pai(self, hu_players, fp_index=-1):
        ...
        DEBUG_MSG('[RoomType22 id %i]------------->HuOpt huIndex=%s, fangPaoIndex=%s' % (self.id, hu_players, fp_index))
        self.end_round()
    ...
    def on_account_cell_destroy(self, account_db_id):
        ...
        :return:
        """
        DEBUG_MSG('RoomType22 cell on_account_cell_destroy account_db_id:%s' % account_db_id)
        _chapter = self.get_current_chapter()
        all_death = True
        for p in _chapter[PLAYER_IN_GAME].values():
            if p['entity']:
    ...
    def can_it_hu(self, shou_pais, pai=-1, hu_result=None):
        ...
        # 七星判断
        if self.room_cfg.is_seven_pair_can_hu:
            ret = mj.RoomType22Calculator.is_seven_pair(new_show_pais)
            if ret:
        ...
        if self.room_cfg.hun_pai_type == 0:
            # 不带混
            ret = mj.RoomType22Calculator.can_hu_contain_lai_zi(new_show_pais, [])
        elif self.room_cfg.hun_pai_type == 1:
            # 单混
            hun_zi_list = self.current_chapter.hun_pais
            hun_pai_count = mj.RoomType22Calculator.hun_pai_count(shou_pais, hun_zi_list)
            if hun_pai_count == 4:
                ret = True
                temp_hu_result = HU_RESULT_FOUR_HUN
            else:
                ret = mj.RoomType22Calculator.can_hu_contain_lai_zi(shou_pais, hun_zi_list, pai)

        elif self.room_cfg.hun_pai_type == 2:
            # 双混, 只能自摸
            if pai == -1:
                hun_zi_list = self.current_chapter.hun_pais
                ret = mj.RoomType22Calculator.can_hu_contain_lai_zi(new_show_pais, hun_zi_list, pai)
    ...
    def is_allow_hu_more(self):
        """
        123,246,515:不允许点炮时胡多张
        点炮、自摸：允许胡多张
        0: 点炮        1：自摸        2: 123        3: 246        4: 515
        """
        game_type = self.room_cfg.hu_type
    ...
    def can_hu_cards_range(self, hand_cards):
        ...
        :return:
        """
        hand_cards = mj.RoomType22Calculator.analyse1(hand_cards)
        card_range = []
        mj.RoomType22Calculator.select_perhaps_pai(hand_cards, card_range)
