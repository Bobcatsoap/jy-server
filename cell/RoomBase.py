...
class RoomBase(KBEngine.Entity):
    ...
    # 跑得快总结算抽水
    def pdk_total_settlement_billing(self, billing_count):
        chapter = self.chapters[self.cn]
    ...
    def jh_get_settlement_winners(self):
       ...
       """
        ...
        DEBUG_MSG(self.chapters[self.cn]['playerInGame'].items())
        for k, v in self.chapters[self.cn]['playerInGame'].items():
            DEBUG_MSG("玩家%s 分数: %s" % (str(v["entity"].info["name"]), str(-v["totalBet"])))
    ...
    def jh_get_true_gold(self, account_id):
        ...
        DEBUG_MSG(self.chapters[self.cn]['playerInGame'].items())
        for k, v in _chapter['playerInGame'].items():
            DEBUG_MSG("金花 玩家%s 分数: %s" % (str(v["entity"].info["name"]), str(-v["totalBet"])))
            if v['entity'].id == account_id:
                return -v["totalBet"]


    def jh_total_settlement_gold(self, account_id):
        """
        金花，获得玩家当前真实金币
        :param account_id:
        :return:
    ...
    def nn_get_total_settlement_gold(self, account_id):
        """
       牛牛，获得玩家当前输赢金币
       :param account_id:
       :return:
    ...
    def nn_total_settlement_billing(self, billing_count):
        ...
        for k, v in total_settlement_winner.items():
            ...
            # 计算大赢家小局抽水
            total_settlement_winner_true_gold = self.nn_get_total_settlement_gold(k)
            DEBUG_MSG('RoomType4  大局抽水  玩家 %s 真实金币%s' % (str(v["entity"].info["name"]), total_settlement_winner_true_gold))
            DEBUG_MSG('RoomType4  大局抽水 抽水比例 %s' % self.info['totalSettlementBilling'])
            total_settlement_winner_billing = total_settlement_winner_true_gold * self.info['totalSettlementBilling']
    ...
    # 麻将总结算抽水
    def mj_total_settlement_billing(self, billing_count):
        chapter = self.chapters[self.cn]
