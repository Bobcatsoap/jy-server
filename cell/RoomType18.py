...
import mj.RoomType18Calculator
import time
import Account
...
# 27 28 29 30                --> 东 南 西 北
# 31 32 33                   --> 中 發 白
from mj import RoomType18Calculator
...
OPT_HU_ZIMO_V = 4
# 听牌
OPT_TING = "resOutPaiTingOpt"
OPT_TING_V = 5
# 听牌放水
OPT_TING_FANG_SHUI = "resOutPaiTingFangShuiOpt"
OPT_TING_FANG_SHUI_V = 6
...
OPT_HU_DIAN_PAO_V = 14
# 抢杠和
OPT_HU_QIANG_GANG = "resOutPaiQiangGangHuOpt"
OPT_HU_QIANGGANG_V = 15
...
OPT_DICT = {OPT_OUT: OPT_OUT_V,
            ...
            OPT_TING: OPT_TING_V,
            OPT_TING_FANG_SHUI: OPT_TING_FANG_SHUI_V,
...
# 听牌
CHECK_TING = 'checkTing'
# 听牌放水
CHECK_TING_FANG_SHUI = 'checkTingFangShui'
...
# 玩家是否听牌 -1.没听  1.听   2.放水听
TING_TYPE = 'tingType'
# 听出牌后
TING_STAGE= 'tingStage'
# 允许客户端的操作
ALLOW_OPT = "allowOpt"
# 牌型
PAI_TYPES = "paiTypes"
...
CLIENT_FUNC_TING = 'tingOptPrompt'
CLIENT_FUNC_TING_FANG_SHUI = 'tingFangShuiPrompt'
# func: gOptPrompt args: 无
CLIENT_FUNC_GUO = "gOptPrompt"
# 听牌提醒
CLIENT_FUNC_TING_TIP = 'tpOptPrompt'
...
class RoomType18(RoomBase):
    ...
    def newChapter(self, maxPlayerCount=100, dissolveDeadLine=-1, dissolveTimer=-1):
        ...
        _chapter[IS_DIAN_GANG_HU] = False
        _chapter[DIAN_GANG_PLAYER] = -1
        # 定时器相关
        _chapter[BOT_OUT_PAI_TIMER] = -1
        _chapter['resOutPaiTimer'] = -1
        _chapter['autoPlayCardAfterTingTimer'] = -1
        # 抢杠胡计时器
        ...
        _chapter['qiangDaMingGangPlayer'] = -1
        ...
        _chapter[HU_TYPE] = -1
        _chapter[IS_GANG_SHANG_PAO] = False
        if self.cn == 0:
    ...
    def onEnter(self, accountEntityId):
        ...
        self.debug_msg('onEnter account = %i.' % _accountEntity.id)
        ...
        self.debug_msg('newPlayer %s' % _player)
        # 如果是此房间第一个玩家进入，默认为房主
        ...
        self.debug_msg('onenter %s %s %s' % (_chapter['chapterState'], len(_chapter[SEAT]), self.started))
    ...
    def onLeave(self, accountEntityId, leave_param=None):
        ...
        self.notify_viewing_hall_players_room_info()
        self.debug_msg('onLeave account = %i.' % accountEntityId)
    ...
    def resort_location(self):
        ...
        self.debug_msg('_chapter[SEAT]:%s' % _chapter[SEAT])
        self.debug_msg('_chapter[SEAT_TO_PLAYER]%s' % _chapter[SEAT_TO_PLAYER])
        self.debug_msg('_chapter[PLAYERS]%s' % _chapter[PLAYER_IN_GAME])
        new_players = _chapter[PLAYER_IN_GAME].copy()
    ...
    def removePlayer(self, _player):
        ...
        if _player:
            ...
            self.debug_msg('removePlayer locationIndex %s,players:%s' % (_locationIndex, _players))
    ...
    def newPlayer(self, accountEntity, total_gold_change=0, base_sync_gold_change=0, agree_disband=-1):
        ...
        self.debug_msg('new player totalGoldChange %s' % total_gold_change)
        ...
        _player[CAN_TING_PAIS] = []
        _player[TING_TYPE] = -1
        # 是否是听完出牌后
        _player[TING_STAGE]=-1
        _player[CAN_MANUAL_PLAY_CARD] = True
        _player[CAN_NOT_PLAY_AFTER_TING] = []
        _player[HU_CARDS] = []
        _player[FAN_COUNT] = 1
        _player[FAN_INFO] = {}
        _player[MOUTH_COUNT] = 0
        _player[MOUTH_INFO] = {}
        _player[LI_ZHANG_COUNT] = 0
        _player[CAN_DI_HU] = True
        _player[CAN_TIAN_HU] = True
        _player[ALL_DEAL_CARDS] = []
        _player[PLAY_AFTER_GANG] = False
    ...
    def addPlayerInSD(self, player):
        ...
        self.debug_msg('addPlayerInSD player:%s.' % player)
    ...
    def on_player_ready(self, entityId, ready=True):
        ...
        _chapter = self.chapters[self.cn]
        # 只有准备阶段可以准备
        if _chapter["chapterState"] != 0 and _chapter["chapterState"] != 6:
            return
        # 如果是比赛场,准备时金币不能小于指定值
        ...
        if _playersCount <= 1:
            return
        # 判断牌局是否可以开始
        if _playersCount < self.info["maxPlayersCount"] and not self.is_less_mode:
            return
        _flag = True
        for i in _players.values():
            if not i["isReady"]:
    ...
    def changeChapterState(self, state):
        ...
        if _old == CHAPTER_STATE_INIT and state == CHAPTER_STATE_READY:
            ...
            self.debug_msg('changeChapterState to %s' % state)
        ...
        elif (_old == CHAPTER_STATE_READY and state == CHAPTER_STATE_START) or ( _old == CHAPTER_STATE_SETTLEMENT and state == CHAPTER_STATE_START):
            _chapter["chapterState"] = state
            self.debug_msg('changeChapterState to %s' % state)
            self.callOtherClientsFunction("chapterStateChanged", [state])
            if self.cn == 0:
            ...
            if not self.xia_pao:
                # 不带跑,玩家下跑数为0
                for player in _chapter[PLAYER_IN_GAME].values():
            ...
            _chapter["chapterState"] = state
            self.debug_msg('changeChapterState to %s' % state)
            self.delTimer(_chapter['xiaPaoTimer'])
            _chapter['xiaPaoTimer'] = -1
            self.callOtherClientsFunction("chapterStateChanged", [state])
            ...
            if self.info["maxChapterCount"] == self.cn + 1:
                ...
                self.del_auto_play_card_after_ting_timer()
    ...
    def player_xia_pao(self, account_entity_id, xia_pao_count):  # timeout=False
        """
        ...
        self.check_xia_pao_result()

    def check_xia_pao_result(self, timeout=False):
        """
        玩家下跑，检查是否都已下跑
        超时，设置默认不跑
        :param timeout:
        ...
        """
        chapter = self.chapters[self.cn]
        chapter_state = chapter['chapterState']
        if chapter_state != CHAPTER_STATE_XIA_PAO:
            return
        # _chapter = self.current_chapter
        _players = chapter[PLAYER_IN_GAME]

        # 超时用户没选择，自主跑默认设为不跑；固定跑不用选择
        for player in _players.values():
            self.debug_msg('palyer_xia_pao count:%s %s' % (player['entity'].id, player[XIA_PAO_COUNT]))
    ...
    def settlement(self):
        ...
        if not _chapter[IS_HUANG_ZHUANG]:
            for hu_player in hu_players:
                fang_shui_mouth = 0
                # 上听放水嘴数
                if hu_player[TING_TYPE] == 2:
                    fang_shui_mouth = self.fang_shui_mouth
                # 乘番数时嘴数要减去立张、下跑和放水
                calculate_mouth_count = hu_player[MOUTH_COUNT] - hu_player[XIA_PAO_COUNT] - \
                                        hu_player[LI_ZHANG_COUNT] - fang_shui_mouth
                self.debug_msg('hu_player:%s,calculate_mouth_count:%s,fan_count:%s' % (hu_player['entity'].info['name'],
                                                                                       calculate_mouth_count,
                                                                                       hu_player[FAN_COUNT]))
                self.debug_msg('hu_player ALL_DEAL_CARDS %s' % hu_player[ALL_DEAL_CARDS])
                # 胡牌分数
                hu_score = hu_player[FAN_COUNT] * calculate_mouth_count
                # 加上放水嘴数、立张嘴数
                hu_score += hu_player[LI_ZHANG_COUNT] + fang_shui_mouth
                # 存储输胡牌分的玩家
                hu_lose_range = []
                # 计算胡牌分数和扣分范围
                if _chapter[IS_ZI_MO_HE]:
                    hu_lose_range = no_hu_players.copy()
                elif _chapter[IS_DIAN_GANG_HU]:
                    # 点杠胡，扣所有输家的分
                    hu_lose_range = no_hu_players.copy()
                    # 点杠胡包大，扣点杠人所有输家的分
                    if self.dian_gang_bao_da:
                        hu_lose_range = no_hu_players.copy()
                elif _chapter[IS_QIANG_GANG_HE]:
                    # 抢杠胡，扣放炮人的分
                    hu_lose_range = [self.get_player_by_location(_chapter[FANG_PAO_INDEX])]
                    # 抢杠包大，扣枪杠人所有输家的分
                    if self.qiang_gang_bao_da:
                        hu_lose_range = no_hu_players.copy()
                elif _chapter[IS_DIAN_PAO_HE]:
                    hu_lose_range = [self.get_player_by_location(_chapter[FANG_PAO_INDEX])]
                    # 点炮三家付，扣所有输家的分
                    if self.dian_pao_all:
                        hu_lose_range = no_hu_players.copy()
                elif _chapter[IS_HUANG_ZHUANG]:
                    hu_score = 0
                    hu_lose_range = []

                self.debug_msg('hu_player:%s,hu_score:%s' % (hu_player['entity'].info['name'], hu_score))

                # 扣胡牌分
                for _p in hu_lose_range:
                    _hu_score = hu_score + _p[XIA_PAO_COUNT] + hu_player[XIA_PAO_COUNT]
                    # 胡分加上下跑分
                    if self.dian_gang_bao_da and _chapter[IS_DIAN_GANG_HU]:
                        # 点杠大包，扣点杠人的分
                        dian_gang_player = self.get_player_by_location(_chapter[DIAN_GANG_PLAYER])
                        dian_gang_player['goldChange'] -= _hu_score
                        dian_gang_player[HU_GOLD] -= _hu_score
                        hu_player['goldChange'] += _hu_score
                        hu_player[HU_GOLD] += _hu_score
                    elif self.qiang_gang_bao_da and _chapter[IS_QIANG_GANG_HE]:
                        # 抢杠大包，扣杠牌的人
                        da_ming_gang_player = self.get_player_by_location(_chapter['qiangDaMingGangPlayer'])
                        da_ming_gang_player['goldChange'] -= _hu_score
                        da_ming_gang_player[HU_GOLD] -= _hu_score
                        hu_player['goldChange'] += _hu_score
                        hu_player[HU_GOLD] += _hu_score
                    else:
                        _p['goldChange'] -= _hu_score
                        _p[HU_GOLD] -= _hu_score
                        hu_player['goldChange'] += _hu_score
                        hu_player[HU_GOLD] += _hu_score

        # 如果没有荒庄，或者荒庄不荒杠开启，计算杠分
        if not _chapter[IS_HUANG_ZHUANG] or self.not_huang_gang:
            # 扣杠分
            ming_gang_score = self.ming_gang_mouth
            an_gang_score = self.an_gang_mouth
            for _gang_p in _chapter[PLAYER_IN_GAME].values():
                # 大明杠分
                da_ming_gang_score = ming_gang_score
                # 大明杠
                if self.dian_gang_all:
                    # 点杠三家付，大明杠扣所有人
                    # 其他玩家
                    for _ in _gang_p[DA_MING_GANG_S]:
                        self.debug_msg('dian_gang_all')
                ...
                # 小明杠分
                xiao_ming_score = ming_gang_score
                # 小明杠扣点杠的人
                for i in _gang_p[XIAO_MING_GANG_S].values():
                    if self.dian_gang_all:
                        for __p in _chapter[PLAYER_IN_GAME].values():
                            if self.is_same_player(__p, _gang_p):
                                continue
                            __p['goldChange'] -= xiao_ming_score
                            _gang_p['goldChange'] += xiao_ming_score
                            __p['mingGangGold'] -= xiao_ming_score
                            _gang_p['mingGangGold'] += xiao_ming_score
                    else:
                        _p = self.get_player_by_location(i)
                        self.debug_msg('name:%s,xiao ming:%s' % (_p['entity'].info["name"], xiao_ming_score))
                ...
                for _ in _gang_p[AN_GANG_S]:
                    ...
                    for _p in _chapter[PLAYER_IN_GAME].values():
                        if self.is_same_player(_p, _gang_p):
                            continue
                        an_score = an_gang_score
                        self.debug_msg('name:%s,an:%s' % (_p['entity'].info["name"], an_score))
        ...
        for _p in players:
            ...
            self.debug_msg('settlement totalGoldChange%s' % _p['totalGoldChange'])
        ...
        for p in players:
            pm = {"locationIndex": p['locationIndex'],
                  ...
                  'gangGold': p['anGangGold'] + p['mingGangGold'],
                  'huGold': p[HU_GOLD],
                  "goldChange": p['goldChange'],
                  "isHu": self.is_player_in_players(p, hu_players),
                  'fanInfo': p[FAN_INFO],
                  'mouthInfo': p[MOUTH_INFO],
                  'fanCount': p[FAN_COUNT],
                  'mouthCount': p[MOUTH_COUNT],
                  'pao': p[XIA_PAO_COUNT],
                  'fangPaoIndex': _chapter[FANG_PAO_INDEX],
                  "huType": _chapter[HU_TYPE]}
            end_msg.append(pm)
        self.debug_msg('chapterEnd end_msg=%s' % end_msg)
        for p in players:
    ...
    def total_settlement(self, is_disband=False):
        ...
        for sdp in sd[SD_PLAYERS].values():
            ...
            room_end_msg["players"].append({
                ...
                "jiePaoCount": sdp[SD_FANG_PAO_COUNT],
        ...
        players = self.get_all_players_by_seat()
        self.debug_msg('totalSettlement msg=%s' % room_end_msg)
    ...
    def write_chapter_replay(self):
        ...
        for c in range(0, chapter_record_max_count):
            ...
            _replay_data[c]['hunPai'] = [31] if self.magic_red_switch else []
    ...
    def write_chapter_info_to_db(self):
        ...
        _chapter_info = _args
        self.base.cellToBase({"func": "writeChapterInfo", "chapterInfos": _chapter_info})
        ...
        self.debug_msg('writeChapterInfoToDB, chapterInfo %s ' % _chapter_info)
    ...
    def set_base_player_game_coin(self, _player):
        ...
        if self.info['roomType'] != 'gameCoin':
            return
        self.debug_msg('set_base_player_game_coin')
        _chapter = self.chapters[self.cn]
    ...
    def set_base_player_gold(self, _player):
        ...
        if self.info['roomType'] != 'card' and self.info['roomType'] != 'normalGameCoin':
            return
        self.debug_msg('set_base_player_gold')
    ...
    def delete_all_timer(self):
        ...
        self.delTimer(_chapter['autoPlayCardAfterTingTimer'])
        _chapter['autoPlayCardAfterTingTimer'] = -1
    ...
    def new_cards_lib(self):
        ...
        self.debug_msg('new_cards_lib _l%s' % _l)
    ...
    def random_cards_with_time(self, _chapter_lib):
        ...
        # self.debug_msg("len %s,chapter lib%s" % (len(_chapter_lib), _chapter_lib))
    ...
    def reconnection(self, accountEntityId):
        ...
        for p in players.values():
            ...
            users.append({
                ...
                'isReady': p['isReady'],
                'pao': p[XIA_PAO_COUNT],
                'canNotPlayAfterTing': p[CAN_NOT_PLAY_AFTER_TING],
                'tingType': p[TING_TYPE],
                'hunPai': [31] if self.magic_red_switch else [],
        ...
        self.syncChapterNum(self.cn + 1)
        self.debug_msg('reconnection msg:%s' % users)
        ...
        if _chapter[CUR_PLAYER] is not None:
            ...
            if player[DAN_FANG] != -1:
                ...
                _ag = self.check_an_gang(player)
                _isHu = self.can_hu(player, _shouPai)

                # 如果不是天胡也不是地胡，没有听牌就不能胡
                if not player[CAN_TIAN_HU] and not player[CAN_DI_HU]:
                    if player[TING_TYPE] == -1:
                        _isHu = False
                ...
                # 不在听完未出牌阶段
                if len(_xmg) != 0 and player[TING_STAGE] != 0:
                    self.callClientFunction(player["entity"].id, CLIENT_FUNC_XIAOMINGGANG, _xmg)
                # 不在听完未出牌阶段
                if len(_ag) != 0 and player[TING_STAGE] != 0:
                    self.callClientFunction(player["entity"].id, CLIENT_FUNC_ANGANG, _ag)
                if _isHu and player[TING_STAGE] != 0:
                    self.callClientFunction(player["entity"].id, CLIENT_FUNC_ZIMOHU, _isHu)
                if (len(_xmg) != 0 or len(_ag) != 0 or _isHu) and player[TING_STAGE] != 0:
                    self.callClientFunction(player["entity"].id, CLIENT_FUNC_GUO, "")
                # 重连后如果玩家没有听牌并且可以听牌发送听牌按钮
                if len(player[CAN_TING_PAIS]) and player[TING_TYPE] == -1:
                    self.callClientFunction(player["entity"].id, CLIENT_FUNC_TING, "")
                    if self.can_ting_fang_shui:
                        self.callClientFunction(player["entity"].id, CHECK_TING_FANG_SHUI, "")

        if _chapter_state == CHAPTER_STATE_PLAY:
            # 如果有同步吃，碰，大明杠，点炮胡操作，并且是听出牌后
            if player[CHECK_RESULT] is not None and player[TING_STAGE] != 0:
    ...
    def playerOperation(self, accountEntityId, jsonData):
        ...
        :return:
        """
        self.debug_msg('playerOperation accountEntityId:%s jsonData:%s' % (accountEntityId, jsonData))
        ...
        if not _check_player:
            self.debug_msg('is not _check_player')
            return

        # 检查该玩家的操作是否被允许
        _check_opt = self.checkOperation(_player, _operation_name)
        if not _check_opt:
            self.debug_msg('is not _check_opt')
            return

        # 如果允许玩家点炮胡，并且玩家此时选择过牌,则视为玩家弃掉了点炮胡
        if _operation_name == OPT_GUO:
            opt_v = OPT_DICT[OPT_HU_DIAN_PAO]
            if opt_v in _player[ALLOW_OPT]:
                _player['qiDianPao'] = True

        # 如果是听牌、听牌放水，不清空允许操作
        if _operation_name != OPT_TING and _operation_name != OPT_TING_FANG_SHUI:
            _player[ALLOW_OPT].clear()

        # 如果是出牌请求并且玩家不能再手动出牌，返回
        if _operation_name == OPT_OUT:
            if not _player[CAN_MANUAL_PLAY_CARD]:
                return

        # 获取操作码
        opt_code = OPT_DICT[_operation_name]
        # 单数操作码为发牌后操作
        if len(str(opt_code)) == 1:
            self.operation_after_deal_card(_player, _operation_name, _args)
        # 双数操作码为出牌后操作
        elif len(str(opt_code)) == 2:
    ...
    def operation_after_deal_card(self, _player, _operation_name, _args):
        ...
        elif OPT_TING == _operation_name:
            self.del_play_card_timer()
            self.del_auto_play_card_after_ting_timer()
            self.ting(_player, 1)
        elif OPT_TING_FANG_SHUI == _operation_name:
            self.del_play_card_timer()
            self.del_auto_play_card_after_ting_timer()
            self.ting(_player, 2)
        ...
        elif OPT_TING_FANG_SHUI == _operation_name or OPT_TING == _operation_name:
            # 发牌之后的操作
            self.operation_after_deal_card(_player, _operation_name, _args)
        elif OPT_GUO_DEAL == _operation_name:
            self.guo_opt_deal(_player)

    def ting(self, player, ting_type):
        """

        """
        self.debug_msg('ting CurPlayer=%s, ting_type=%s' % (player['locationIndex'], ting_type))
        _chapter = self.get_current_chapter()
        _players = _chapter[PLAYER_IN_GAME]
        _hand_cards = player[SHOU_PAI]
        _entity_id = player["entity"].id
        if not player[CAN_TING_PAIS]:
            self.debug_msg(
                'can not ting CurPlayer=%s, CAN_TING_PAIS=%s' % (player['locationIndex'], player[CAN_TING_PAIS]))
            self.callClientFunction(_entity_id, 'Notice', ['没有打出后可以听的牌'])
            return
        player[TING_TYPE] = ting_type
        player[TING_STAGE]=0
        ting_info = []
        for k, v in _players.items():
            dic = {'entityId': v['entity'].id, 'type': v[TING_TYPE]}
            ting_info.append(dic)
        self.callOtherClientsFunction('playerTing', {'tingInfo': ting_info})
        ...
        if ting_type == 1:
            ...
            opt = 'tingFangShui'
        ting_replay = {'locationIndex': player['locationIndex'], 'otherIndex': -1, "opt": opt, "card": []}
        _chapter['replay'].append(ting_replay)

        _can_not_play_cards_after_ting = player[SHOU_PAI].copy()
        for card in player[CAN_TING_PAIS]:
            while card in _can_not_play_cards_after_ting:
                _can_not_play_cards_after_ting.remove(card)

        # 清空允许操作


        # 发送不能听的牌
        self.callClientFunction(_entity_id, 'canPlayCardsAfterTing', {'cards': _can_not_play_cards_after_ting})
        # 清空能听的牌
        player[CAN_TING_PAIS].clear()
        # 记录听后不能出的牌
        player[CAN_NOT_PLAY_AFTER_TING] = _can_not_play_cards_after_ting.copy()
    ...
    def operation_after_play_card(self, _player, _operation_name, _args):
        ...
        self.debug_msg('out_pai_after_operation operation_name %s' % _operation_name)
        _chapter = self.get_current_chapter()
        ...
        elif OPT_HU_QIANG_GANG == _operation_name:
    ...
    def clientReq(self, accountEntityId, jsonData):
        ...
        self.debug_msg('clientReq accountEntityId%s,jsonData%s' % (accountEntityId, jsonData))
        if _func_name == "LeaveRoom":
            ...
            self.player_xia_pao(accountEntityId, _args["pao"])
    ...
    def retLocations(self):
        ...
        for _p in _players.values():
            self.debug_msg('retLocations players:%s' % _p)
            _e = _p["entity"]
            if _e.info["isBot"] == 0:
                _json_data = json.dumps(_location_info)
                _e.clientEntity(self.info['roomId']).retLocationIndexs(_json_data)
    ...
    def is_all_agree(self):
        ...
        players = _chapter[PLAYER_IN_GAME]
        for p in players.values():
            self.debug_msg('is_all_agree entity_id:%s agree:%s' % (p['entity'].id, p[IS_ARGEE]))
            if p[IS_ARGEE] != 1:
                return False
        return True
    ...
    def onTimer(self, timerId, userArg):
        ...
        if timerId == _timers["main"]:
            ...
            self.check_xia_pao_result(True)
        ...
        elif timerId == _chapter['autoPlayCardAfterTingTimer']:
            self.delTimer(timerId)
            _chapter['autoPlayCardAfterTingTimer'] = -1
            self.auto_play_card_after_ting()
        elif timerId == _chapter['resOutPaiTimer']:
            self.delTimer(timerId)
            _chapter['resOutPaiTimer'] = -1
            # 响应出牌，其他玩家超时后默认操作
            self.debug_msg('Timer(%s) 响应出牌,(%s)超时操作[guoOpt]' % (timerId, self.get_current_player()['locationIndex']))
            self.not_opt_players_operate_pass()
        elif timerId == _chapter['qiangGangTimer']:
            self.delTimer(timerId)
            _chapter['qiangGangTimer'] = -1
            # 响应出牌，其他玩家超时后默认操作
            self.debug_msg('Timer(%s) 响应抢杠胡,(%s)超时操作[guoOpt]' % (timerId, self.get_current_player()['locationIndex']))
            self.not_opt_players_operate_pass()
    ...
    def bless(self, account_entity_id, bless_type):
        ...
        if account_entity.today_bless_count > Const.GameConfigJson.config_json['Hall']['blessRoomCardStandard']:
            ...
            self.debug_msg('remain_room_card:%s' % remain_room_card)
    ...
    def init_shou_pai(self):
        ...
        # 有人输的太多的话，需要发好牌
        is_in_rand_range = self.is_need_rand_score_control("RoomType18")
        luck_player = None
        if is_in_rand_range:
        ...
        if not luck_player:
            ...
            if max_losing_streak_count < 5:
                luck_player = None
            self.debug_msg('最大连输 %s %s' % (max_losing_streak_count, luck_player['entity'].id if luck_player else luck_player))
        ...
        if luck_player:
            ...
            good_card_index = mj.RoomType18Calculator.get_good_pai(all_cards, hun_zi_list)
    ...
    def deal_cards_to_player(self, locationIndex, cards):
        ...
        self.debug_msg('deal_cards_to_player locationIndex:%s,cards:%s' % (locationIndex, cards))
    ...
    def end_round(self):
        ...
        """
        # 开启结算倒计时
        self.get_current_chapter()['settlementTimer'] = self.addTimer(SETTLEMENT_TIME, 0, 0)
    ...
    def notice_next_player(self, player, _round):
        ...
        # 给当前玩家发牌
        self.deal_one_card(self.get_current_player(), is_gang=False)
    ...
    def get_all_players_by_seat(self, start_seat=0):
        ...
        self.debug_msg('get_all_players_by_seat seat_to_player%s' % _chapter[SEAT_TO_PLAYER])
    ...
    def change_cur_player(self, player):
        ...
        if _chapter[CUR_PLAYER]:
            self.debug_msg('ChangeCurPlayer %s to %s' % (_chapter[CUR_PLAYER]['locationIndex'],
                                                         player['locationIndex']))
    ...
    def check_current_player(self, player):
        ...
        if not self.is_player_in_players(player, allow_out_players):
            ...
            self.debug_msg('Error: correct player: %s, current player: %s' % (allow_location_index,
                                                                              player['locationIndex']))
            return False
        return True
    ...
    #                            系统操作集合
    # --------------------------------------------------------------------------------------------
    def deal_one_card(self, player, is_gang=False, is_da_ming_gang=False):
        """
        给某个玩家发一张牌
        :param player:
        :param is_gang:
        ...
        # 是否是杠后出牌
        if is_gang:
            player[PLAY_AFTER_GANG] = True
        else:
            player[PLAY_AFTER_GANG] = False
        _chapter = self.get_current_chapter()
        # 清除不能胡的牌
        ...
        # 记录发过的牌
        player[ALL_DEAL_CARDS].append(pai)
        # 设置单放
        player[DAN_FANG] = pai
        # 操作次数+1
        player[OPT_COUNT] += 1
        ...
        self.debug_msg('deal_one_card hand_cards:%s' % _shou_pai)
        self.ting_tip(player['entity'].id)
        self.syncLeftPaiCount(_chapter[LEFT_PAI_COUNT])
        self.check_can_opt_after_deal(player, is_gang=is_gang, is_da_ming_gang=is_da_ming_gang)

    def check_can_opt_after_deal(self, player, is_gang=False, is_gang_after_peng=False, is_da_ming_gang=False):
        """
        检查发牌后可以进行的操作
        ...
        _ag = self.check_an_gang(player)
        _isHu = self.can_hu(player, _hand_cards)
        self.debug_msg('FaPai ishu=%s' % _isHu)
        # 如果不是天胡也不是地胡，没有听牌就不能胡
        if not player[CAN_TIAN_HU] and not player[CAN_DI_HU]:
            if player[TING_TYPE] == -1:
                _isHu = False
        ...
        # 如果是大明杠后发牌，并且可以胡，那么记为杠上开花
        if is_gang and is_da_ming_gang and _isHu:
            _chapter[IS_DIAN_GANG_HU] = True

        all_not = True

        # 客户端操作验证
        player[ALLOW_OPT].clear()
        player[ALLOW_OPT].add(OPT_OUT_V)
        player[ALLOW_OPT].add(OPT_GUO_DEAL_V)
        # 允许小明杠，通知小明杠按钮
        if len(_xmg) != 0:
            player[ALLOW_OPT].add(OPT_XIAOMINGGANG_V)
            self.callClientFunction(player["entity"].id, CLIENT_FUNC_XIAOMINGGANG, _xmg)
            all_not = False
        # 允许暗杠，通知暗杠按钮
        if len(_ag) != 0:
            player[ALLOW_OPT].add(OPT_ANGANG_V)
            self.callClientFunction(player["entity"].id, CLIENT_FUNC_ANGANG, _ag)
            all_not = False
        # 允许胡牌，通知胡牌按钮
        if _isHu and not is_gang_after_peng:
            player[ALLOW_OPT].add(OPT_HU_ZIMO_V)
            self.callClientFunction(player["entity"].id, CLIENT_FUNC_ZIMOHU, _isHu)
            all_not = False
        # 如果能并且没有听,发送听按钮
        if player[CAN_TING_PAIS] and player[TING_TYPE] == -1:
            player[ALLOW_OPT].add(OPT_TING_V)
            self.callClientFunction(player["entity"].id, CLIENT_FUNC_TING, "")
            if self.can_ting_fang_shui:
                player[ALLOW_OPT].add(OPT_TING_FANG_SHUI_V)
                self.callClientFunction(player["entity"].id, CLIENT_FUNC_TING_FANG_SHUI, "")
            all_not = False

        if not all_not:
            self.callClientFunction(player["entity"].id, CLIENT_FUNC_GUO, "")

        # 如果玩家不在线，并且设置了托管，自动出牌
        if player['entity'].client_death and self.info['timeDown'] != 0:
            # 发牌之后,超时默认操作
            cur_player = self.get_current_player()
            out_pai = cur_player[SHOU_PAI][-1]
            self.operation_after_deal_card(cur_player, OPT_OUT, [out_pai])
        # 如果没有任何可用操作（杠、胡）并且听了，直接出牌
        elif player[TING_TYPE] != -1 and all_not:
            _chapter['autoPlayCardAfterTingTimer'] = self.addTimer(AUTO_PLAY_CARD_AFTER_TING, 0, 0)
            _chapter['deadLine'] = time.time() + AUTO_PLAY_CARD_AFTER_TING
            # 标记玩家为不可以再主动出牌
            player[CAN_MANUAL_PLAY_CARD] = False
        # 如果玩家在线，开启计时器
        else:
            # 开启出牌定时器
    ...
    def auto_play_card_after_ting(self):
        """
        听牌后的自动出牌
        """
        # 发牌之后,超时默认操作
    ...
    def operation_over(self, timeout=False, operator=None, operation_name=None):
        ...
        self.debug_msg('operation_over is_qiang_gang_hu%s' % is_qiang_gang_hu)
        # 检查是否有人大明杠
        da_ming_gang_player = None
        for cr in check_results:
            _cpo = cr[CHECK_PLAYER_OPT]
            if _cpo == OPT_DAMINGGANG_V:
                da_ming_gang_player = cr[CHECK_PLAYER]
                break
        for cr in check_results:
            _cpo = cr[CHECK_PLAYER_OPT]
            # 判断操作是否是胡牌
            if _cpo == OPT_HU_DIAN_PAO_V or _cpo == OPT_HU_QIANGGANG_V:
                if _cpo == OPT_HU_QIANGGANG_V:
                    qiang_gang_he = True
                # 如果有人大明杠的同时有人点炮胡，则大明杠者是输家
                if _cpo == OPT_HU_DIAN_PAO_V and da_ming_gang_player:
                    qiang_gang_he = True
                    _chapter['qiangDaMingGangPlayer'] = da_ming_gang_player['locationIndex']
                    self.debug_msg('operation_over qiangDaMingGangPlayer:%s' % _chapter['qiangDaMingGangPlayer'])
                can_other_opt = False
                # 找到所有的胡牌玩家
    ...
    def del_auto_play_card_after_ting_timer(self):
        _chapter = self.get_current_chapter()
        self.delTimer(_chapter['autoPlayCardAfterTingTimer'])
        _chapter['autoPlayCardAfterTingTimer'] = -1
    ...
    def del_res_play_card_timer(self):
        _chapter = self.get_current_chapter()
        self.delTimer(_chapter['resOutPaiTimer'])
        self.debug_msg('删除响应出牌超时定时器(%s)' % _chapter['resOutPaiTimer'])
        _chapter['resOutPaiTimer'] = -1
    ...
    def play_card(self, player, out_pai):
        ...
        if out_pai in _hand_cards:
            ...
            # 出牌失败，允许操作重新赋值
            player[ALLOW_OPT].add(OPT_OUT_V)
            self.callClientFunction(player['entity'].id, 'refreshChapter', {})
            return
        _chapter[CUR_PAI] = out_pai
        self.get_current_player()[OUT_PAIS].append(_chapter[CUR_PAI])
        # 如果听牌并且开启立张了，记录立张数
        if player[TING_TYPE] != -1 and self.li_zhang_switch:
            player[LI_ZHANG_COUNT] += 1
        # 同步出牌消息
        self.syncOpt(player['locationIndex'], SYNCOPT_OUTPAI, out_pai, -1)
        # 设置听出牌后
        if player[TING_TYPE] > 0:
            player[TING_STAGE] = 1
        # 设置能胡的牌
        self.init_hu_cards(player)
        # 设置不胡牌
        self.set_not_hu_pai(player)
        # 清除杠上花标记
        self.clearFaiPaiDate()
        # 出牌后不可天湖
        player[CAN_TIAN_HU] = False
        # 玩家操作，检查过参数之后，删除系统超时操作
        self.del_play_card_timer()
        self.del_auto_play_card_after_ting_timer()
        # 其他玩家；从当前玩家开始，要有顺序
        ...
        for i, p in enumerate(other_players):
            ...
            can_da_ming_gang = self.check_da_ming_gang(p, out_pai)

            b_hu = self.can_hu(p, p[SHOU_PAI], pai=out_pai)
            # 如果不是天胡也不是地胡，没有听牌就不能胡
            if not p[CAN_TIAN_HU] and not p[CAN_DI_HU]:
                if p[TING_TYPE] == -1:
                    b_hu = False
            ...
            p[ALLOW_OPT].clear()
            if len(_chi) != 0:
                p[ALLOW_OPT].add(OPT_CHI_V)
            # 听过之后不能碰
            if p[CAN_PENG]:
                p[ALLOW_OPT].add(OPT_PENG_V)
            if can_da_ming_gang:
                p[ALLOW_OPT].add(OPT_DAMINGGANG_V)
            # 如果没有弃胡，则可以点炮胡
            if p[CAN_HU] and self.can_dian_pao(p, out_pai):
                p[ALLOW_OPT].add(OPT_HU_DIAN_PAO_V)
            if p[ALLOW_OPT]:
                p[ALLOW_OPT].add(OPT_GUO_V)
        ...
        self.debug_msg('CHECK_RESULTS:%s' % _chapter[CHECK_RESULTS])
    ...
    def xiao_ming_gang_opt(self, player, pai):
        self.debug_msg('xiaoMingGangOpt CurPlayer=%s, Pai=%s' % (player['locationIndex'], pai))
        ...
        # 再次发送胡牌提示
        self.hu_tip(player['entity'].id)
        # 给此玩家发牌
        self.deal_one_card(player, is_gang=True)



    def an_gang_opt(self, player, pai):
        self.debug_msg('anGangOpt CurPlayer=%s, Pai=%s' % (player['locationIndex'], pai))
        # 移除杠牌
    ...
    def zi_mo_hu_opt(self, player):
        self.debug_msg('ziMoHuOpt CurPlayer=%s' % player['locationIndex'])
        _chapter = self.get_current_chapter()
        _chapter[IS_ZI_MO_HE] = True
        _chapter[HU_TYPE] = HU_TYPE_ZI_MO
        if _chapter[IS_DIAN_GANG_HU]:
            _chapter[HU_TYPE] = HU_TYPE_DIAN_GANG
        # 同步自摸消息
    ...
    def check_qiang_gang_hu(self, player, pai):
        ...
        for op_player in other_players:
            can_hu = self.can_hu(op_player, op_player[SHOU_PAI], pai=pai)
            # 如果不是天胡也不是地胡，没有听牌就不能胡
            if not op_player[CAN_TIAN_HU] and not op_player[CAN_DI_HU]:
                if op_player[TING_TYPE] == -1:
                    can_hu = False
            ...
            # 用户检查客户端操作
            op_player[ALLOW_OPT].clear()
            if op_player[CAN_HU] and self.can_dian_pao(op_player, pai):
                op_player[ALLOW_OPT].add(OPT_HU_QIANGGANG_V)
                op_player[ALLOW_OPT].add(OPT_GUO_V)
    ...
    def chiOpt(self, player, pai):
        self.debug_msg('chiOpt CurPlayer=%s, Pais=%s' % (player['locationIndex'], pai))
        _chapter = self.get_current_chapter()
    ...
    def peng_opt(self, player, pai):
        self.debug_msg('pengOpt CurPlayer=%s, Pai=%s' % (player['locationIndex'], pai))
    ...
    def res_play_card_da_ming_gang_opt(self, player, pai):
        ...
        :return:
        """
        self.debug_msg('resOutPaiDaMingGangOpt CurPlayer=%s, Pai=%s' % (player['locationIndex'], pai))
        _chapter = self.get_current_chapter()
        _check_results = _chapter[CHECK_RESULTS]
        for cr in _check_results:
            if player['entity'].id == cr[CHECK_PLAYER]['entity'].id:
    ...
    def da_ming_gang_opt(self, player, pai):
        self.debug_msg('daMingGangOpt CurPlayer=%s, Pai=%s' % (player['locationIndex'], pai))
        self.check_opt_pai(pai)
        _chapter = self.get_current_chapter()
        other_index = self.get_wait_player()['locationIndex']
        ...
        # 记录大明杠玩家
        _chapter[DIAN_GANG_PLAYER] = other_index
        # 大明杠后不可地胡
        player[CAN_DI_HU] = False
        # 销毁check_results
        self.destroy_check_result(player)
        ...
        self.hu_tip(player['entity'].id)
        # 给此玩家发牌
        self.deal_one_card(player, is_gang=True, is_da_ming_gang=True)
    ...
    def res_out_pai_dian_pao_hu_opt(self, player):
        ...
        :return:
        """
        self.debug_msg('resOutPaiDianPaoHuOpt CurPlayer=%s' % player['locationIndex'])
        _chapter = self.get_current_chapter()
        _check_results = _chapter[CHECK_RESULTS]
        for cr in _check_results:
            if player['entity'].id == cr[CHECK_PLAYER]['entity'].id:
    ...
    def dian_pao_hu_opt(self, players):
        self.debug_msg('dianPaoHuOpt CurPlayer=%s' % players)
        # 要确保多个点炮胡操作顺序执行
        _chapter = self.get_current_chapter()
        # 如果没开一炮多响，只保留一个胡牌玩家
        if not self.more_ring:
            can_hu = self.not_more_ring_filter(players)
            if can_hu:
                players = can_hu
        _chapter[IS_DIAN_PAO_HE] = True
        fp_player = self.get_wait_player()
        # 杠后出牌算杠上炮
        if fp_player[PLAY_AFTER_GANG]:
            _chapter[IS_GANG_SHANG_PAO] = True
    ...
    def not_more_ring_filter(self, hu_players):
        """
        过滤胡牌玩家，如果一炮多响没有开启，离点炮者最近的为胡牌玩家
        :param hu_players:
        :return:
        """
        if not self.more_ring:
            if len(hu_players) > 1:
                chapter = self.get_current_chapter()
                fang_pao_index = self.get_wait_player()['locationIndex']
                min_distance = sys.maxsize
                min_location_index = -1
                for hu_player in hu_players:
                    if hu_player['locationIndex'] > fang_pao_index:
                        distance = hu_player['locationIndex'] - fang_pao_index
                    else:
                        distance = hu_player['locationIndex'] - fang_pao_index + len(chapter[PLAYER_IN_GAME])
                    if distance < min_distance:
                        min_distance = distance
                        min_location_index = hu_player['locationIndex']

                self.debug_msg('not_more_ring_filter %s,%s' % (min_distance, min_location_index))
                if min_location_index != -1:
                    _copy = hu_players.copy()
                    for _p in _copy:
                        if _p['locationIndex'] == min_location_index:
                            hu_players = [_p]
                            return hu_players
    ...
    def res_out_pai_qiang_gang_hu_opt(self, player):
        ...
        :return:
        """
        self.debug_msg('resOutPaiQiangGangHuOpt CurPlayer=%s' % player)
        _chapter = self.get_current_chapter()
        _check_results = _chapter[CHECK_RESULTS]
        for cr in _check_results:
            if player['entity'].id == cr[CHECK_PLAYER]['entity'].id:
    ...
    def qiang_gang_hu_opt(self, players):
        self.debug_msg('qiangGangHuOpt CurPlayer=%s' % players)
        _chapter = self.get_current_chapter()
        # 如果没开一炮多响，只保留一个胡牌玩家
        if not self.more_ring:
            can_hu = self.not_more_ring_filter(players)
        ...
        # 如果是大明杠抢杠胡,将打出的牌加入手牌
        if _chapter['qiangDaMingGangPlayer'] != -1:
            qiang_gang_card = _chapter[CUR_PAI]
        # 抢杠胡，将胡的牌加入手中
        for player in players:
    ...
    def hu(self, hu_players, fp_index=-1):
        ...
        self.init_fan_and_mouth(hu_players)
        self.end_round()

    def init_fan_and_mouth(self, hu_players):
        """
        通过牌型获取嘴数和番数
        @param hu_players:
        @return:
        ...
        for player in hu_players:
            RoomType18Calculator.init_fan_and_mouth(self.info, _chapter, player)

    def init_hu_cards(self, player):
        """
        能胡的牌赋值
        :return:
        @param player: 玩家信息字典
        ...
        for card in self.card_range:
            ...
            temp.append(card)
            # 平胡
            can_hu = self.can_hu(player, temp)
            if can_hu:
                tip_cards.append(card)

        player[HU_CARDS] = tip_cards

    def hu_tip(self, account_entity_id, play_card=-1):
        """
        胡牌提示
        :param account_entity_id:
        :param play_card:
        ...
        if player['entity'].id != account_entity_id:
            return
        if player[TING_TYPE] == -1 and not player[CAN_TIAN_HU] and not player[CAN_DI_HU]:
            return
        hand_cards = player[SHOU_PAI].copy()
        self.debug_msg('hu_tip hand_cards%s' % hand_cards)
        tip_cards = []
        ...
        for card in tip_cards:
            un_reveal_count = self.get_card_count_un_reveal(card, exclude_player=account_entity_id)
            if un_reveal_count > 0:
                card_info[str(card)] = un_reveal_count
    ...
    def check_allow_dian_pao_hu(self, player):
        ...
        """
        opt_v = OPT_DICT[OPT_HU_DIAN_PAO]
        if opt_v in player[ALLOW_OPT]:
            return True
        return False

    @property
    def dian_gang_all(self):
        """
        是否开启点杠三家付
        """
        return self.info['dianGangAll']

    @property
    def only_zi_mo(self):
        # 有 1 可点炮胡
        return 1 not in self.info['specialRules']

    @property
    def li_zhang_switch(self):
        """
        立张开关
        :return:
        """
        return 3 in self.info['specialRules']

    @property
    def can_ting_fang_shui(self):
        return self.info['fangShui'] > 0

    @property
    def fang_shui_mouth(self):
        return self.info['fangShui']
    ...
    def xia_pao(self):
        ...
        """
        return 2 in self.info['specialRules']

    @property
    def not_huang_gang(self):
        return self.info['notHuangGang']

    @property
    def ming_gang_mouth(self):
        return 1

    @property
    def an_gang_mouth(self):
        return 2
    ...
    def more_ring(self):
        ...
        :return:
        """
        return self.info['moreRing']

    @property
    def qiang_gang_bao_da(self):
        """
        抢杠包大
        :return:
        """
        return self.info['qiangGangBaoDa']

    @property
    def dian_gang_bao_da(self):
        return self.info['dianGangBaoDa']

    @property
    def dian_pao_all(self):
        return self.info['dianPaoAll']
    ...
    def on_account_cell_destroy(self, account_db_id):
        ...
        :return:
        """
        self.debug_msg('on_account_cell_destroy account_db_id:%s' % account_db_id)
        _chapter = self.get_current_chapter()
        all_death = True
        for p in _chapter[PLAYER_IN_GAME].values():
            if p['entity']:
    ...
    def get_special_multiple(self, special):
        ...
        :return:
        """
        # 特殊规则：1：七星对  2：豪华七星对  3:双豪华七星对  4：三豪华七星对
        if special == 1:
            return 4
        elif special == 2:
            return 6
    ...
    def can_hu(self, player, cards, cards_type=None, pai=-1):
        """
        判胡
        :param player:
        :param pai: 别人打出的牌
        ...
        # 两人模式必须缺一门才能胡牌
        # if self.info['maxPlayersCount'] == 2:
        #     # all_cards = RoomType18Calculator.get_all_hand_cards(player)
        #     # all_cards.append(pai)
        #     if RoomType18Calculator.get_duan_men_count(new_cards) < 1:
        #         self.debug_msg('can_hu not duan men 1')
        #         return
        if self.magic_red_switch:
            wild_hu, j_258 = mj.RoomType18Calculator.can_hu_contain_lai_zi(cards, [31], pai)
        else:
            normal_hu, j_258 = mj.RoomType18Calculator.can_hu_contain_lai_zi(cards, [], pai)
        # 平胡
        hu = normal_hu or wild_hu
        if hu:
            hu_type = 0
        # 七星对
        seven_hu = mj.RoomType18Calculator.is_seven_pair(new_cards)
        if seven_hu > 0:
            hu_type = seven_hu
            hu = True
        # 引用传递胡牌类型
    ...
    def magic_red_switch(self):
        return self.info['wildCardType'] == 1
    ...
    def ting_tip(self, account_entity_id):
        ...
        # 胡牌最多的听牌
        hu_most = {}
        can_hu_range = self.can_hu_cards_range(_hand_cards)
        self.debug_msg('ting_tip can_hu_range:%s' % can_hu_range)
        # 1.移除要出的牌      2.加入任意牌     3.移除杠牌      4.判断能不能胡    5.判断能不能次
        for play_card in _hand_cards:
            ...
            self.debug_msg('can_ting checked_cards:%s,check_card:%s' % (checked_cards, play_card))
            hand_cards = player[SHOU_PAI].copy()
            ...
            if play_card in hand_cards:
                ...
                for card in can_hu_range:
                    ...
                    cards_type = []
                    multiple = 1
                    can_hu = self.can_hu(player, temp, cards_type=cards_type)
        ...
        player[CAN_TING_PAIS] = ting_tip_cards

        self.debug_msg('onEnter 听牌结果：%s' % ting_tip_cards)
        self.callClientFunction(player['entity'].id, CLIENT_FUNC_TING_TIP, {'tingCards': ting_tip_cards,
                                                                            'huMost': hu_most_list,
                                                                            'huMax': hu_max_list})

    def hu_check(self, player, cards):
        """
        当前牌能胡什么牌
        :param player:
        :return:
    ...
    def can_hu_cards_range(self, hand_cards):
        ...
        :return:
        """
        hand_cards = mj.RoomType18Calculator.analyse(hand_cards)
        card_range = []
        mj.RoomType18Calculator.select_perhaps_pai(hand_cards, card_range, self.magic_red_switch)
    ...
    def check_an_gang(self, player):
        ...
        # 如果玩家听牌，只能对不影响手牌的牌进行暗杠
        if player[TING_TYPE] != -1:
            for x in an_gang.copy():
                _check_cards = player[SHOU_PAI].copy()
                _check_cards.remove(x)
                _check_cards.remove(x)
                _check_cards.remove(x)
                _check_cards.remove(x)
                if not self.hu_check(player, _check_cards) or x != _chapter[CUR_PAI]:
                    an_gang.remove(x)
        return an_gang

    def check_da_ming_gang(self, player, check_card):
        """
        判断是否能大明杠
        :param hand_cards: 手牌
        ...
        if count >= 3:
            # 如果玩家听牌，只能对不影响手牌的牌进行大明杠
            if player[TING_TYPE] != -1:
                _check_cards = player[SHOU_PAI].copy()
                if check_card in _check_cards:
                    _check_cards.remove(check_card)
                    _check_cards.remove(check_card)
                    _check_cards.remove(check_card)
                    if not self.hu_check(player, _check_cards):
                        return False
            return True
        else:
            return False
    ...
    def check_xiao_ming_gang(self, hand_cards, pengs, player):
        ...
        # 如果玩家已听牌，检测小明杠后是否依旧能听牌
        if player[TING_TYPE] != -1:
            for x in ret.copy():
                # 如果玩家可以小明杠并且是摸到的牌
                if x != _chapter[CUR_PAI]:
                    continue
                _check_cards = player[SHOU_PAI].copy()
                if x in _check_cards:
                    _check_cards.remove(x)
                    if not self.hu_check(player, _check_cards):
                        ret.remove(x)
        return ret
    ...
    def is_same_player(self, player1, player2):
        if 'entity' in player1 and 'entity' in player2 and player1['entity'].id == player2['entity'].id:
            return True
        return False
