...
class HTTPResponse:
    ...
    def end(self):
        if not self._complete:
            ...
            INFO_MSG('[MinHTTPServer] end body %s' % self.body)
...
class MinHTTPServer:
    ...
    def onAccept(self, fileno):
        if self._sock.fileno() == fileno:
            INFO_MSG('[MinHTTPServer] onAccept self._sock fileno %s' % fileno)
            sock, addr = self._sock.accept()
            INFO_MSG('[MinHTTPServer] onAccept addr %s:%s' % addr)
            KBEngine.registerReadFileDescriptor(sock.fileno(), Functor.Functor(self.onRecv, sock, addr))

    def onRecv(self, sock, addr, fileno):
        INFO_MSG('[MinHTTPServer] onRecv sock fileno %s' % sock.fileno())
            ...
            if req.url.startswith(k):
                for func in v:
                    try:
                        ...
                        INFO_MSG('[MinHTTPServer] onRecv except pass %s %s' % (k, e))
                        pass
                    finally:
                        INFO_MSG('[MinHTTPServer] onRecv finally')
                        if resp.complete():
                            return
        if self._resPath:
    ...
    def onRespStaticRes(self, req, resp):
        INFO_MSG('[MinHTTPServer] onRespStaticRes')
