...
def onInterfaceAppReady():
    ...
    INFO_MSG('[interface]: start poller port = %s' % 30429)
    g_poller.start("localhost", 30429)
...
def onRequestCreateAccount(registerName, password, datas):
    ...
    INFO_MSG('[kbemain] onRequestCreateAccount: registerName=%s password=%s datas=%s' % (registerName, password, datas))
...
def onRequestAccountLogin(loginName, password, datas):
    ...
    INFO_MSG('[kbemain] onRequestAccountLogin: registerName=%s password=%s datas=%s' % (loginName, password, datas))
